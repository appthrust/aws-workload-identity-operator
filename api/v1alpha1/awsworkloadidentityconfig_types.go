...
type AWSWorkloadIdentityConfigStatus struct {
	...
	// PublishedIssuerHash is a fingerprint of the OIDC discovery and JWKS
	// objects last uploaded to the self-hosted issuer S3 bucket. The
	// controller re-publishes when this value does not match the hash of the
	// freshly rendered objects, so renderer-format changes and any input
	// change that flows into the rendered output (issuer URL, signing key,
	// bucket) trigger a re-publish even when the key ID is unchanged.
	// +optional
	// +kubebuilder:validation:MaxLength=128
	PublishedIssuerHash string `json:"publishedIssuerHash,omitempty"`
