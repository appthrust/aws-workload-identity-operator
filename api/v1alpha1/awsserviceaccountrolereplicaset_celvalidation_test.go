...
// maxAggregateAnnotationBytes mirrors the aggregate-annotation-size CEL rule on
// TemplateMetadata.Annotations: apimachinery's TotalAnnotationSizeLimitB
// (262144) minus 1024 bytes reserved for operator-stamped annotations.
const maxAggregateAnnotationBytes = 261120

func TestTemplateMetadataAggregateAnnotationSize(t *testing.T) {
	// The 18-byte key uses the "example.com/ann-%02d" template; each row's
	// aggregate is entries*(18+valueLen), which the test re-derives so the
	// expected total stays a function of the row parameters.
	cases := []struct {
		name         string
		entries      int
		valueLen     int
		...
		// 64 * (18 + 4083) = 262464 bytes (> 261120). Each value (4083 bytes)
		// is well under the per-value MaxLength of 262144, so only the
		// aggregate-annotation-size CEL rule fires.
		{name: "rejects over budget", entries: 64, valueLen: 4083, expectReject: true},
		// 60 * (18 + 3982) = 240000 bytes, 21120 under the cap so the
		// boundary is not knife-edge sensitive.
		{name: "accepts under budget", entries: 60, valueLen: 3982, expectReject: false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			aggregate := tc.entries * (18 + tc.valueLen)

			if tc.expectReject && aggregate <= maxAggregateAnnotationBytes {
				t.Fatalf("test setup error: aggregate %d does not exceed cap %d", aggregate, maxAggregateAnnotationBytes)
			}

			if !tc.expectReject && aggregate >= maxAggregateAnnotationBytes {
				t.Fatalf("test setup error: aggregate %d is not under cap %d", aggregate, maxAggregateAnnotationBytes)
			}

			filler := strings.Repeat("x", tc.valueLen)
			annotations := make(map[string]interface{}, tc.entries)

			for i := 0; i < tc.entries; i++ {
				annotations[fmt.Sprintf("example.com/ann-%02d", i)] = filler
			}
			...
			if tc.expectReject {
				if len(errs) == 0 {
					t.Fatalf("expected aggregate-annotation-size CEL rule to reject %d bytes, got no errors", aggregate)
				}

				if !containsErr(errs, fmt.Sprintf("%d", maxAggregateAnnotationBytes)) {
					t.Fatalf("expected error mentioning the %d-byte aggregate cap, got %v", maxAggregateAnnotationBytes, errs)
				}
			...
			if len(errs) != 0 {
				t.Fatalf("expected aggregate %d bytes (under %d cap) to be accepted, got %v",
					aggregate, maxAggregateAnnotationBytes, errs)
			}
		})
	}
}
