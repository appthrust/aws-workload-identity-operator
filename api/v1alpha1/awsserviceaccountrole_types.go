...
type AWSServiceAccountRoleStatus struct {
	...
	// identifier used for remote ServiceAccount annotation cleanup on
	// annotation-based IRSA delivery (`SelfHostedIRSA` and `EKSIRSA`). The
	// `EKSPodIdentity` delivery does not annotate the remote ServiceAccount
	// and leaves this field empty. The controller encodes the identifier as
	// `<namespace>/<name>` via `types.NamespacedName.String()`, where both
	// segments are DNS-1123 subdomains (and are currently equal — see
	// `logicalClusterName` in `internal/inventory/resolver.go`). The bound
	// covers two maximum-length subdomain segments and the separating slash
	// (253 + 1 + 253).
	// +optional
