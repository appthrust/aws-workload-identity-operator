...
type AWSServiceAccountRoleSpec struct {
	...
	// PolicyDocument is the raw JSON encoding of an inline customer-managed IAM
	// policy document attached to the generated role. The MaxLength bound mirrors
	// AWS's customer-managed policy size limit (6144 characters), which keeps the
	// stored object bounded per api-conventions.md and lets admission reject
	// payloads that would not fit at the AWS API.
	// +optional
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:MaxLength=6144
