...
// that requires exactly one of spec.policyARNs / spec.policyDocument.
//
...
func TestRoleSpecRejectsBothPolicyARNsAndDocument(t *testing.T) {
	...
	// simultaneously and produce silently divergent IAM delivery. The
	// XOR rule
	// `(has(self.policyARNs) && self.policyARNs.size() > 0) != has(self.policyDocument)`
	// must reject a spec that supplies both fields.
	spec := map[string]interface{}{
		"serviceAccount": validServiceAccount(),
