...
const (
	testWriterKarpenterRoleARN = "arn:aws:iam::123456789012:role/karpenter"
	testWriterWorkloadRoleARN  = "arn:aws:iam::123456789012:role/workload"
)
...
func TestWriteTokenAtomicWritesFinalFileWithSecureMode(t *testing.T) {
	...
	data, err := os.ReadFile(path) //nolint:gosec // test reads a temp file path created by this test.
	if err != nil {
...
func TestWriteAWSConfigAtomicWritesDefaultProfile(t *testing.T) {
	...
	err := WriteAWSConfigAtomic(path, AWSConfig{ //nolint:gosec // Static IAM ARN fixture values, not credentials.
		RoleARN:              testWriterKarpenterRoleARN,
	...
	want := `[default]
role_arn = ` + testWriterKarpenterRoleARN + `
...
func TestWriteAWSConfigAtomicRequiresRoleSessionName(t *testing.T) {
	...
	err := WriteAWSConfigAtomic(path, AWSConfig{ //nolint:gosec // Static IAM ARN fixture values, not credentials.
		RoleARN:              testWriterWorkloadRoleARN,
		WebIdentityTokenFile: "/var/run/aws-irsa/token",
	})
	if err == nil {
