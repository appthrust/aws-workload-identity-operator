...
func TestRefreshInfersServiceAccountAndWritesTokenAndAWSConfig(t *testing.T) {
	...
	remoteClient := &fakeRemoteClient{
		...
		serviceAccounts: map[types.NamespacedName]*corev1.ServiceAccount{
			{Namespace: "app", Name: "workload"}: serviceAccount("app", "workload", map[string]string{
	...
	if remoteClient.selfSubjectReviewCalls != 1 {
		t.Fatalf("SelfSubjectReview calls = %d, want 1", remoteClient.selfSubjectReviewCalls)
	}
	if remoteClient.getServiceAccountCalls[0] != (types.NamespacedName{Namespace: "app", Name: "workload"}) {
		t.Fatalf("Get ServiceAccount = %s", remoteClient.getServiceAccountCalls[0])
	}
	if len(remoteClient.tokenRequests) != 1 {
		t.Fatalf("TokenRequest calls = %d, want 1", len(remoteClient.tokenRequests))
	}
	tokenRequest := remoteClient.tokenRequests[0]
	...
	awsConfigData, err := os.ReadFile(awsConfigFile)
	if err != nil {
...
role_session_name = ` + roleSessionName(remoteConfig, types.NamespacedName{Namespace: "app", Name: "workload"}) + `
...
func TestRefreshRejectsMissingRoleARNAnnotation(t *testing.T) {
	remoteClient := &fakeRemoteClient{
		username: "system:serviceaccount:app:workload",
		serviceAccounts: map[types.NamespacedName]*corev1.ServiceAccount{
			{Namespace: "app", Name: "workload"}: serviceAccount("app", "workload", nil),
		},
	}
	agent := directTestAgent(t, time.Now(), remoteClient)
...
func TestRefreshRejectsMissingLocalConfigurationBeforeRemoteCalls(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: "AWSConfigFile is required",
		},
	}
...
func serviceAccount(namespace, name string, annotations map[string]string) *corev1.ServiceAccount {
