...
func (p *Provider) Retrieve(ctx context.Context) (aws.Credentials, error) { //nolint:funlen // Linear credential exchange is clearer in one flow.
	...
	if resolved.DeliveryType != string(identityv1.DeliveryTypeSelfHostedIRSA) {
	...
	region := resolveSTSRegion(p.opts.Region, clusterProfile.AWSRegion, resolved.Region)
	if region == "" {
...
func resolveSTSRegion(explicit, clusterProfile, config string) string {
	if explicit != "" {
		return explicit
	}
