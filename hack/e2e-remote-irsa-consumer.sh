...
REMOTE_IRSA_TOKEN_PERMISSIONS_NAME="awio-e2e-remote-irsa-tokenrequest"
...
cleanup() {
  ...
  if hub_kubeconfig_ready; then
    hub_kubectl -n "$WLC_NAMESPACE" delete manifestwork "$REMOTE_IRSA_TOKEN_PERMISSIONS_NAME" --ignore-not-found=true --timeout=60s >/dev/null 2>&1 || true
...
build_remote_irsa_helper() {
  log "building aws-remote-irsa-credential-process helper"
  (cd "$ROOT_DIR" && GOWORK=off go build -o "$HELPER_BINARY" ./cmd/aws-remote-irsa-credential-process)
}
...
install_cp_creds_plugin() {
  ...
  docker cp "${container_id}:/plugins/cp-creds" "$cp_creds"
...
write_helper_provider_file() {
  jq -n \
    --arg command "${WORK_DIR}/bin/cp-creds" \
...
kind: ManifestWork
metadata:
  name: ${REMOTE_IRSA_TOKEN_PERMISSIONS_NAME}
  namespace: ${WLC_NAMESPACE}
spec:
  workload:
    manifests:
...
EOF
  ...
  wait_for_json_condition_hub "$WLC_NAMESPACE" manifestwork "$REMOTE_IRSA_TOKEN_PERMISSIONS_NAME" Applied "$TIMEOUT_SECONDS"
}
...
verify_tokenrequest_authorization() {
  ...
  managed_kubectl auth can-i create serviceaccounts/token \
    --resource-name "$APP_SERVICE_ACCOUNT" \
    -n "$APP_NAMESPACE" \
    --as "$REMOTE_ACCESS_SUBJECT" | grep -qx yes

  managed_kubectl auth can-i create serviceaccounts/token \
    --resource-name "$MISMATCH_SERVICE_ACCOUNT" \
    -n "$APP_NAMESPACE" \
    --as "$REMOTE_ACCESS_SUBJECT" | grep -qx yes

  managed_kubectl auth can-i create serviceaccounts/token \
    --resource-name "$DENIED_SERVICE_ACCOUNT" \
    -n "$APP_NAMESPACE" \
    --as "$REMOTE_ACCESS_SUBJECT" | grep -qx no
...
run_remote_irsa_helper() {
  ...
  "$HELPER_BINARY" \
    --kubeconfig "$HUB_KUBECONFIG" \
    --namespace "$WLC_NAMESPACE" \
    --service-account "${APP_NAMESPACE}/${service_account}" \
    --aws-service-account-role "$ROLE_RESOURCE_NAME" \
    --session-name "awio-e2e-${RUN_ID}-${service_account}" \
    --clusterprofile-provider-file "$PROVIDER_FILE" \
    >"$output_file"
}
...
verify_remote_irsa_consumer() {
  local caller_json credentials_file forbidden_err mismatched_err role_name sts_err start
  ...
  forbidden_err="${WORK_DIR}/remote-irsa-forbidden.err"
  if run_remote_irsa_helper "$DENIED_SERVICE_ACCOUNT" "${WORK_DIR}/denied-creds.json" 2>"$forbidden_err"; then
    log "helper unexpectedly succeeded for ServiceAccount without tokenrequest RBAC"
    return 1
  fi
  if ! grep -Eiq 'forbidden|cannot create|serviceaccounts/token' "$forbidden_err"; then
    log "forbidden negative check did not fail with an expected Kubernetes RBAC error"
    sed 's/^/[e2e] forbidden: /' "$forbidden_err" >&2 || true
    return 1
  fi
  log "negative RBAC check verified"

  mismatched_err="${WORK_DIR}/remote-irsa-mismatch.err"
  if run_remote_irsa_helper "$MISMATCH_SERVICE_ACCOUNT" "${WORK_DIR}/mismatch-creds.json" 2>"$mismatched_err"; then
    log "helper unexpectedly succeeded for ServiceAccount outside the IAM trust policy"
    return 1
  fi
