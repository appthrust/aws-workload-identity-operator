...
capture_aws_cleanup_identifiers() {
  ...
  value="$(hub_kubectl -n "$WLC_NAMESPACE" get awsworkloadidentityconfig default -o jsonpath='{.status.bucketName}' 2>/dev/null || true)"
...
verify_remote_irsa_consumer() {
  ...
  wait_for_nonempty_hub_jsonpath "AWSWorkloadIdentityConfig bucket name" "$TIMEOUT_SECONDS" \
    hub_kubectl -n "$WLC_NAMESPACE" get awsworkloadidentityconfig default -o jsonpath='{.status.bucketName}'
  ...
  [[ -n "$BUCKET_NAME" ]] || die "AWSWorkloadIdentityConfig.status.bucketName is required for cleanup verification"
