...
TIMEOUT_SECONDS="${E2E_TIMEOUT_SECONDS:-1200}"
HELM_TIMEOUT="${E2E_HELM_TIMEOUT:-15m}"

HUB_KIND_CLUSTER_NAME="${E2E_HUB_KIND_CLUSTER_NAME:-awio-hub-${RUN_ID}}"
KIND_NODE_IMAGE="${E2E_KIND_NODE_IMAGE:-kindest/node:v1.35.0}"
DOCKER_SOCK="${E2E_DOCKER_SOCK:-}"
KUBECONFIG_MERGE_TARGET="${E2E_KUBECONFIG_MERGE_TARGET:-}"
HUB_KUBECONFIG=""
MANAGED_KUBECONFIG=""
HUB_CONTEXT=""
MANAGED_CONTEXT=""
...
ACK_NAMESPACE="${E2E_ACK_NAMESPACE:-ack-system}"
AWIO_NAMESPACE="${E2E_AWIO_NAMESPACE:-awio-system}"
AWIO_RELEASE="${E2E_AWIO_RELEASE:-awio}"
AWIO_FULLNAME="${AWIO_RELEASE}-aws-workload-identity-operator"
AWIO_IMAGE="${E2E_AWIO_IMAGE:-ghcr.io/appthrust/aws-workload-identity-operator:e2e}"
AWIO_IMAGE_REGISTRY=""
AWIO_IMAGE_REPOSITORY=""
AWIO_IMAGE_TAG=""

CERT_MANAGER_VERSION="${E2E_CERT_MANAGER_VERSION:-v1.19.1}"
CLUSTERADM_VERSION="${E2E_CLUSTERADM_VERSION:-latest}"
CAPD_PROVIDER="${E2E_CAPD_PROVIDER:-docker}"
CAPD_FLAVOR="${E2E_CAPD_FLAVOR:-development}"
CAPD_KUBERNETES_VERSION="${E2E_CAPD_KUBERNETES_VERSION:-v1.35.0}"
CNI_MANIFEST_URL="${E2E_CNI_MANIFEST_URL:-https://raw.githubusercontent.com/projectcalico/calico/v3.30.3/manifests/calico.yaml}"

WLC_NAMESPACE="${E2E_WORKLOAD_CLUSTER_NAMESPACE:-wlc-${RUN_ID}}"
APP_NAMESPACE="${E2E_APP_NAMESPACE:-irsa-${RUN_ID}}"
APP_SERVICE_ACCOUNT="${E2E_APP_SERVICE_ACCOUNT:-sts-canary}"
ROLE_RESOURCE_NAME="${E2E_ROLE_RESOURCE_NAME:-sts-canary}"
OCM_ACCESS_PROVIDER="${E2E_OCM_ACCESS_PROVIDER:-open-cluster-management}"
MANAGED_SERVICE_ACCOUNT="${E2E_MANAGED_SERVICE_ACCOUNT:-aws-workload-identity-operator}"
AWS_CLI_IMAGE="${E2E_AWS_CLI_IMAGE:-public.ecr.aws/aws-cli/aws-cli:2.31.11}"
POD_IDENTITY_WEBHOOK_IMAGE="${E2E_POD_IDENTITY_WEBHOOK_IMAGE:-public.ecr.aws/eks/amazon-eks-pod-identity-webhook:v0.6.14}"

KEEP_HUB="${E2E_KEEP_HUB:-0}"
KEEP_WORKLOAD_CLUSTER="${E2E_KEEP_WORKLOAD_CLUSTER:-0}"
...
wait_for_clusterprofile() {
  ...
  start="$(date +%s)"
  while true; do
    if json="$(hub_kubectl -n "$AWIO_NAMESPACE" get clusterprofile "$WLC_NAMESPACE" -o json 2>/dev/null)"; then
      ...
      if [[ "$ready" == "True" && "$provider_count" != "0" ]]; then
        log "ClusterProfile ${AWIO_NAMESPACE}/${WLC_NAMESPACE} is ready with ${OCM_ACCESS_PROVIDER} access"
        return 0
      fi
    fi

    if (( $(date +%s) - start > TIMEOUT_SECONDS )); then
      log "ClusterProfile ${AWIO_NAMESPACE}/${WLC_NAMESPACE} did not become usable"
      hub_kubectl -n "$AWIO_NAMESPACE" get clusterprofile "$WLC_NAMESPACE" -o yaml || true
      return 1
    fi
...
wait_for_clusterprofile_credential() {
  ...
  secret_name="${WLC_NAMESPACE}-${MANAGED_SERVICE_ACCOUNT}"
  start="$(date +%s)"
  while true; do
    token="$(hub_kubectl -n "$AWIO_NAMESPACE" get secret "$secret_name" -o jsonpath='{.data.token}' 2>/dev/null || true)"
    if [[ -n "$token" ]]; then
      log "ClusterProfile credential secret ${AWIO_NAMESPACE}/${secret_name} is ready"
      return 0
    fi

    if (( $(date +%s) - start > TIMEOUT_SECONDS )); then
      log "ClusterProfile credential secret ${AWIO_NAMESPACE}/${secret_name} was not created"
      hub_kubectl -n "$AWIO_NAMESPACE" get secret "$secret_name" -o yaml || true
      return 1
    fi
...
capture_aws_resource_ids() {
  if hub_kubeconfig_ready; then
    if [[ -z "$ROLE_ARN" ]]; then
      ROLE_ARN="$(hub_kubectl -n "$WLC_NAMESPACE" get awsserviceaccountrole "$ROLE_RESOURCE_NAME" -o jsonpath='{.status.roleARN}' 2>/dev/null || true)"
    fi
    if [[ -z "$ROLE_NAME" && -n "$ROLE_ARN" ]]; then
      ROLE_NAME="${ROLE_ARN##*/}"
    fi
    if [[ -z "$OIDC_PROVIDER_ARN" ]]; then
      OIDC_PROVIDER_ARN="$(hub_kubectl -n "$WLC_NAMESPACE" get awsworkloadidentityconfig default -o jsonpath='{.status.oidcProviderARN}' 2>/dev/null || true)"
    fi
  fi
}
...
cleanup() {
  ...
  if [[ "$SKIP_CLEANUP" == "1" ]]; then
    log "E2E_SKIP_CLEANUP=1; leaving resources in place"
    exit "$exit_code"
  fi
  ...
  capture_aws_resource_ids
  if hub_kubeconfig_ready; then
    hub_kubectl -n "$WLC_NAMESPACE" delete awsserviceaccountrole "$ROLE_RESOURCE_NAME" --ignore-not-found=true --timeout=90s >/dev/null 2>&1 || true
    wait_for_hub_delete "$WLC_NAMESPACE" awsserviceaccountrole "$ROLE_RESOURCE_NAME" 180 || true
    hub_kubectl -n "$WLC_NAMESPACE" delete awsworkloadidentityconfig default --ignore-not-found=true --timeout=90s >/dev/null 2>&1 || true
    wait_for_hub_delete "$WLC_NAMESPACE" awsworkloadidentityconfig default 180 || true
    hub_kubectl -n "$WLC_NAMESPACE" delete managedserviceaccount "$MANAGED_SERVICE_ACCOUNT" --ignore-not-found=true >/dev/null 2>&1 || true
    hub_kubectl -n "$WLC_NAMESPACE" delete manifestwork awio-e2e-operator-msa-rbac --ignore-not-found=true >/dev/null 2>&1 || true
  ...
  if [[ "$KEEP_WORKLOAD_CLUSTER" == "1" ]]; then
    log "E2E_KEEP_WORKLOAD_CLUSTER=1; workload cluster ${WLC_NAMESPACE} kept with context ${MANAGED_CONTEXT:-<unresolved>} in ${KUBECONFIG_MERGE_TARGET}"
  ...
  if [[ "$KEEP_WORKLOAD_CLUSTER" != "1" && "$CAPD_PROVIDER" == "docker" ]]; then
  ...
  if [[ "$KEEP_HUB" != "1" ]]; then
    ...
    log "E2E_KEEP_HUB=1; hub kind cluster ${HUB_KIND_CLUSTER_NAME} kept with context ${HUB_CONTEXT:-<unresolved>} in ${KUBECONFIG_MERGE_TARGET}"
  fi
...
preflight() {
  ...
  if [[ -z "$DOCKER_SOCK" ]]; then
    case "${DOCKER_HOST:-}" in
      ...
      "")
        ...
        ;;
      *)
        die "hub kind cluster requires a local Docker unix socket; set E2E_DOCKER_SOCK"
        ;;
    esac
  fi
  ...
  if ! command -v clusteradm >/dev/null 2>&1; then
    ...
    log "installing clusteradm ${CLUSTERADM_VERSION} into ${WORK_DIR}/bin"
    GOBIN="${WORK_DIR}/bin" go install "open-cluster-management.io/clusteradm/cmd/clusteradm@${CLUSTERADM_VERSION}"
  fi
...
parse_operator_image() {
  ...
  if [[ "$image_without_tag" == "$AWIO_IMAGE" ]]; then
    die "E2E_AWIO_IMAGE must include a tag"
  fi
...
patch_capd_manifest_for_irsa() {
  ...
  yq -y --explicit-start \
    ...
    --arg work "$WORK_DIR" \
    --arg cluster "$WLC_NAMESPACE" \
    '
      if .kind == "KubeadmControlPlaneTemplate" and .metadata.name == "quick-start-control-plane" then
      ...
      elif .kind == "Cluster" and .metadata.name == $cluster then
        .spec.topology.variables =
          ((.spec.topology.variables // []) | map(if .name == "podSecurityStandard" then (.value.enabled = false) else . end))
      else
        .
      end
...
register_managed_cluster_with_ocm() {
  local token_json token hub_apiserver
  ...
  clusteradm_cmd join \
    ...
    --capi-import \
    --capi-cluster-name "${WLC_NAMESPACE}/${WLC_NAMESPACE}" \
    --capi-kubeconfig "$HUB_KUBECONFIG" \
...
configure_managed_service_account() {
  cat <<EOF | hub_kubectl apply -f -
...
kind: ManifestWork
metadata:
  name: awio-e2e-operator-msa-rbac
  namespace: ${WLC_NAMESPACE}
spec:
  workload:
    manifests:
...
verify_selfhosted_irsa() {
  ...
  BUCKET_NAME="$(hub_kubectl -n "$WLC_NAMESPACE" get awsworkloadidentityconfig default -o jsonpath='{.status.bucketName}')"
