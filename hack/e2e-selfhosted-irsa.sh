...
OCM_REMOTE_PERMISSIONS_NAME="${E2E_OCM_REMOTE_PERMISSIONS_NAME:-awio-e2e-operator-remote-permissions}"
...
cleanup() {
  ...
  capture_aws_resource_ids
  if hub_kubeconfig_ready; then
    if hub_kubectl -n "$WLC_NAMESPACE" delete awsserviceaccountrole "$ROLE_RESOURCE_NAME" --ignore-not-found=true --timeout=90s >/dev/null 2>&1 &&
      wait_for_hub_delete "$WLC_NAMESPACE" awsserviceaccountrole "$ROLE_RESOURCE_NAME" 180 &&
      hub_kubectl -n "$WLC_NAMESPACE" delete awsworkloadidentityconfig default --ignore-not-found=true --timeout=90s >/dev/null 2>&1 &&
      wait_for_hub_delete "$WLC_NAMESPACE" awsworkloadidentityconfig default 180; then
