...
| `IAMProviderReady` | The IAM OpenIDConnectProvider is synced by ACK, or an external EKSIRSA provider ARN path has been checked against the issuer. |
| `IssuerReady` | Issuer resources for the selected delivery type are ready. |
| `WebhookRuntimeReady` | Self-hosted webhook runtime readiness on the target cluster. `True/NotRequired` for `EKSPodIdentity` and `EKSIRSA`. |
...
| `ServiceAccountAnnotationReady` | Remote ServiceAccount annotations are synced for `SelfHostedIRSA` and `EKSIRSA`. |
...
| `DeletionBlocked` | Role deletion is waiting for remote ServiceAccount annotation cleanup or for owned ACK children (PodIdentityAssociation, IAM Role, generated IAM Policy) to be deleted. |
...
`status.rollout.conditions` and `status.placements[].rollout.conditions` carry
per-rollout signals that drive the top-level `PlacementRolledOut` condition.

| Condition | Meaning |
| --- | --- |
| `Progressing` | `True` with reason `Progressing` while the OCM rollout still has clusters updating, failed, or timed out. Flips to `False` with reason `Complete` once every selected cluster reports `Succeeded` and no children are failed, timed out, or updating. |
