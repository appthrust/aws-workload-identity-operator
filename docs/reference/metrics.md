...
| `awio_watch_map_list_errors_total` | Counter | `controller`, `map_func`, `kind` | LIST errors raised inside controller-runtime `EnqueueRequestsFromMapFunc` callbacks. `MapFunc` cannot return errors, so a failed LIST silently drops the source watch event; this counter exposes that drop rate so on-call can alert before events go missing. |
...
| `map_func` | Map-function identifier passed by the controller (for example `rolesForConfig`, `rolesForOperatorConfig`, `rolesForClusterProfile`, `rolesForAckChildCRD`, `rolesForSiblingServiceAccountBinding`, `configsForOperatorConfig`, `configsForAckChildCRD`, `replicaSetsForNamespace`, `replicaSetsForOCMPlacement`, `replicaSetsForOCMPlacementDecision`). Values are passed through `stableLabel` and coerced to `other` if longer than 64 characters or containing characters outside `[A-Za-z0-9_.-]`. |
| `kind` | Watch-source Kubernetes Kind whose LIST failed (for example `AWSWorkloadIdentityConfig`, `AWSWorkloadIdentityOperatorConfig`, `AWSServiceAccountRole`, `ClusterProfile`, `CustomResourceDefinition`, `Namespace`, `Placement`, `PlacementDecision`). Subject to the same `stableLabel` bounding as `map_func`. |
| `condition` | Kubernetes condition type strings under 64 safe characters, otherwise `other`. |
| `status` | `True`, `False`, `Unknown`, or `other`. |
| `reason` | Stable API condition reasons plus remote delivery reasons: `waiting_inventory`, `delivery_type_mismatch`, `cluster_unavailable`, `no_inventory_namespace`, `apply_failed`, `index_lookup_failed`, `enqueued`, `channel_full`, and `stale_cluster_event`. |
...
Watch map LIST failures silently dropping enqueue events:

```promql
sum by (controller, map_func, kind) (
  rate(awio_watch_map_list_errors_total[5m])
)
```
