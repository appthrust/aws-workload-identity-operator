...
| `spec.policyDocument` | Exactly one of `policyARNs` or `policyDocument` | Yes | Raw JSON encoding of an inline IAM policy document, capped at 6144 characters to match AWS's customer-managed policy size limit. The operator creates an ACK IAM Policy for it. |
