...
small direct S3 permission surface to verify, publish, and delete issuer
objects.

## SelfHostedIRSA Manager Policy

Attach this policy to the IAM role assumed by the operator manager Pod. The
manager itself calls the AWS S3 API to verify, write, and delete only these two
issuer objects:
...
the manager role does not need bucket create or bucket policy permissions for
this path. S3 authorizes `HeadObject` through `s3:GetObject`, so the manager
needs `s3:GetObject` on the two issuer object ARNs.
