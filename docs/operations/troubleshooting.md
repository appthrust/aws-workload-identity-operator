...
- Give the manager IAM role `s3:PutObject` and `s3:DeleteObject` only for
  `.well-known/openid-configuration` and `keys.json` in generated issuer
  buckets.
