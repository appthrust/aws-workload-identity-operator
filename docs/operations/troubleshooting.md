...
- If the matching `ClusterProfile` lives outside the operator install
  namespace, set `clusterInventory.clusterProfileNamespaces` to that namespace
  list, or to `["*"]` only when cluster-wide watch is required.
