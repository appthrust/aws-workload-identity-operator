...
flowchart LR
  ...
  subgraph target["Target cluster"]
    ...
    irsa["SelfHostedIRSA/EKSIRSA annotations"]
...
ACK owns the S3 Bucket CR and AWS bucket lifecycle for `SelfHostedIRSA`. The
operator manager writes the two public OIDC issuer objects into that bucket with
the AWS S3 API. For `EKSIRSA`, EKS provides the OIDC issuer and the operator
delivers identity by writing normal IRSA annotations to the remote
`ServiceAccount`.
