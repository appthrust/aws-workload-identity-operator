...
func TestParseOptionsHelpFlagWritesUsage(t *testing.T) {
	for _, flagName := range []string{"--help", "-h"} {
		t.Run(flagName, func(t *testing.T) {
			...
			usage := out.String()
			if !strings.Contains(usage, "-kubeconfig") {
				t.Fatalf("usage missing -kubeconfig flag: %q", usage)
			}

			if !strings.Contains(usage, "-token-file") {
				t.Fatalf("usage missing -token-file flag: %q", usage)
			}
		})
	}
}

func TestRunCheckRequiresNonEmptyTokenFileAndAWSConfigFile(t *testing.T) {
	dir := t.TempDir()
	tokenFile := filepath.Join(dir, "token")
	awsConfigFile := filepath.Join(dir, "aws-config")

	if got := runCheck([]string{"--token-file", tokenFile, "--aws-config-file", awsConfigFile}, io.Discard); got != 1 {
	...
	if got := runCheck([]string{"--token-file", tokenFile}, io.Discard); got != 1 {
		t.Fatalf("runCheck missing AWS config flag = %d, want 1", got)
	}
	...
	if got := runCheck([]string{"--token-file", tokenFile, "--aws-config-file", awsConfigFile}, io.Discard); got != 0 {
...
func TestRunCheckRejectsUnexpectedPositionalArgument(t *testing.T) {
	...
	if got := runCheck([]string{"--token-file", tokenFile, "extra"}, io.Discard); got != 1 {
		t.Fatalf("runCheck positional argument = %d, want 1", got)
	}
}

func TestRunCheckHelpFlagWritesUsage(t *testing.T) {
	for _, flagName := range []string{"--help", "-h"} {
		t.Run(flagName, func(t *testing.T) {
			var out bytes.Buffer

			if got := runCheck([]string{flagName}, &out); got != 0 {
				t.Fatalf("runCheck help exit = %d, want 0", got)
			}
			...
			if !strings.Contains(usage, "-aws-config-file") {
				t.Fatalf("usage missing -aws-config-file flag: %q", usage)
			}
		})
	}
}
