...
func parseOptions(args []string) (remoteirsacredentialprocess.Options, error) {
	...
	if err := fs.Parse(args); err != nil {
		return remoteirsacredentialprocess.Options{}, fmt.Errorf("parse flags: %w", err)
	}
	if fs.NArg() > 0 {
	...
	if opts.WorkloadNamespace == "" {
		return remoteirsacredentialprocess.Options{}, fmt.Errorf("--namespace is required")
	}
	if opts.Access.ProviderFile == "" {
		return remoteirsacredentialprocess.Options{}, fmt.Errorf("--clusterprofile-provider-file is required")
	}
	if opts.SessionName == "" {
		return remoteirsacredentialprocess.Options{}, fmt.Errorf("--session-name is required")
	}
	if opts.TokenExpiration <= 0 {
		return remoteirsacredentialprocess.Options{}, fmt.Errorf("--token-expiration must be greater than zero")
	}
	if opts.TokenExpiration < remoteirsacredentialprocess.MinimumTokenExpiration {
		return remoteirsacredentialprocess.Options{}, fmt.Errorf("--token-expiration must be at least 10m")
	}
	if opts.SessionDuration < 0 {
		return remoteirsacredentialprocess.Options{}, fmt.Errorf("--session-duration must be greater than or equal to zero")
	}
