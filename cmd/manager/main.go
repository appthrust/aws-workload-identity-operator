...
func init() {
	...
	// CustomResourceDefinition is watched by the optional ACK CRD detection
	// path in config_controller / role_controller; client-go's scheme does
	// not include apiextensions, so register it explicitly.
...
func newMulticlusterManager(provider *inventory.Provider, opts *managerOptions) (mcmanager.Manager, error) {
	cacheByObject, err := localCacheByObject(opts)
	if err != nil {
		return nil, err
	}
...
func parseFlags() managerOptions {
	...
	flag.StringVar(&opts.clusterProfileNamespaces, "cluster-profile-namespaces", "", "Comma-separated namespaces to watch for ClusterProfiles. Use '*' to watch all namespaces.")
...
func validateOptions(opts *managerOptions) error {
	if _, err := clusterProfileCacheByObject(opts); err != nil {
		return err
	}
...
// options. ClusterProfile scope is operator-configurable; Secret stripping and
// Namespace managed-fields stripping live in separate factories so each can be
// tested independently.
func localCacheByObject(opts *managerOptions) (map[client.Object]cache.ByObject, error) {
	result := make(map[client.Object]cache.ByObject)

	clusterProfileCache, err := clusterProfileCacheByObject(opts)
	if err != nil {
		return nil, err
	}

	mergeCacheByObjectByGVK(result, clusterProfileCache, scheme)
...
func clusterProfileCacheByObject(opts *managerOptions) (map[client.Object]cache.ByObject, error) {
	namespaces, allNamespaces, err := normalizeClusterProfileNamespaces(opts.clusterProfileNamespaces)
	if err != nil {
		return nil, err
	}
	if !allNamespaces && len(namespaces) == 0 {
		return nil, fmt.Errorf("--cluster-profile-namespaces is required")
	}

	return map[client.Object]cache.ByObject{
		&clusterinventoryv1alpha1.ClusterProfile{}: {
			Namespaces: namespaceCacheConfigs(namespaces, allNamespaces),
		},
	}, nil
}

func normalizeClusterProfileNamespaces(raw string) ([]string, bool, error) {
	...
	for _, item := range strings.Split(raw, ",") {
		...
		if namespace == "" {
			...
			return nil, false, fmt.Errorf("--cluster-profile-namespaces must not contain empty entries")
		}

		if namespace == allNamespacesSentinel {
			allNamespaces = true
		...
		if errs := validation.IsDNS1123Label(namespace); len(errs) > 0 {
			return nil, false, fmt.Errorf("--cluster-profile-namespaces entry %q must be a valid namespace name: %s", namespace, strings.Join(errs, "; "))
		}
	...
	if allNamespaces {
		if len(namespaces) > 0 {
			return nil, false, fmt.Errorf("--cluster-profile-namespaces must be either \"*\" or explicit namespaces, not both")
		}
...
func namespaceCacheConfigs(namespaces []string, allNamespaces bool) map[string]cache.Config {
	if allNamespaces {
...
func setupControllers(mcMgr mcmanager.Manager, opts *managerOptions) error {
	...
	if err := localMgr.GetFieldIndexer().IndexField(context.Background(), &identityv1.AWSServiceAccountRole{}, controller.IndexRoleByReplicaSetOwnerRef, controller.IndexAWSServiceAccountRoleByReplicaSetOwnerRef); err != nil {
		return fmt.Errorf("register AWSServiceAccountRole ReplicaSet owner-ref field index: %w", err)
	}
