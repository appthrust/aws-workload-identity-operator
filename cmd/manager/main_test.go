...
func TestValidateOptionsAcceptsHTTPSAWSEndpointURL(t *testing.T) {
	opts := &managerOptions{awsEndpointURL: "https://aws-endpoint.example.com", clusterProfileNamespaces: "awio-system"}
...
func TestValidateOptionsAllowsHTTPOnlyWhenExplicit(t *testing.T) {
	opts := &managerOptions{awsEndpointURL: "http://aws-endpoint.example.com", clusterProfileNamespaces: "awio-system"}
...
func TestValidateOptionsRejectsInvalidAWSEndpointURL(t *testing.T) {
	...
	for _, endpoint := range tests {
		opts := &managerOptions{
			...
			clusterProfileNamespaces:  "awio-system",
		}
...
func TestValidateOptionsRequiresClusterProfileNamespaces(t *testing.T) {
	opts := &managerOptions{}

	if err := validateOptions(opts); err == nil {
		t.Fatal("expected --cluster-profile-namespaces to be required")
	}

	opts.clusterProfileNamespaces = "team-a"
	if err := validateOptions(opts); err != nil {
		t.Fatalf("expected explicit ClusterProfile namespace to satisfy validation, got %v", err)
	}

	opts.clusterProfileNamespaces = "*"
	if err := validateOptions(opts); err != nil {
		t.Fatalf("expected wildcard ClusterProfile namespace to satisfy validation, got %v", err)
	}
}

func TestClusterProfileCacheByObjectScopesNamespaces(t *testing.T) {
	tests := []struct {
		name           string
		opts           managerOptions
		...
		{
			name:           "trims blanks and deduplicates explicit namespaces",
			opts:           managerOptions{clusterProfileNamespaces: " team-a,team-b,team-a "},
			wantNamespaces: []string{"team-a", "team-b"},
		},
		{
			name:    "wildcard watches all namespaces",
			opts:    managerOptions{clusterProfileNamespaces: "*"},
			wantAll: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			byObject, err := clusterProfileCacheByObject(&tt.opts)
			if err != nil {
				t.Fatal(err)
			}

			clusterProfileCache, ok := cacheByObjectForGVK(t, testManagerScheme(t), byObject, &clusterinventoryv1alpha1.ClusterProfile{})
			if !ok {
				t.Fatal("expected ClusterProfile cache entry")
			}

			if tt.wantAll {
				if clusterProfileCache.Namespaces == nil || len(clusterProfileCache.Namespaces) != 0 {
					t.Fatalf("expected explicit all-namespace cache scope, got %#v", clusterProfileCache.Namespaces)
				}

				return
			}

			assertNamespaceCacheNames(t, clusterProfileCache.Namespaces, tt.wantNamespaces)
		})
	}
}

func TestClusterProfileCacheByObjectRejectsInvalidNamespaces(t *testing.T) {
	tests := []struct {
		name string
		opts managerOptions
	}{
		...
		{
			name: "empty entry",
			opts: managerOptions{clusterProfileNamespaces: "team-a,,team-b"},
		},
		{
			name: "wildcard mixed with explicit namespace",
			opts: managerOptions{clusterProfileNamespaces: "team-a,*"},
		},
		{
			name: "invalid namespace",
			opts: managerOptions{clusterProfileNamespaces: "Team_A"},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := clusterProfileCacheByObject(&tt.opts); err == nil {
...
func TestWithWebhookRuntimeCacheScopeInitializesCacheOptions(t *testing.T) {
	...
	if countObjectGVK(t, options.Scheme, options.Client.Cache.DisableFor, &corev1.ServiceAccount{}) != 1 {
		t.Fatalf("expected DisableFor to include remote ServiceAccount live reads, got %#v", options.Client.Cache.DisableFor)
	}

	if countObjectGVK(t, options.Scheme, options.Client.Cache.DisableFor, &corev1.Secret{}) != 0 {
		t.Fatalf("expected webhook runtime Secrets to use the scoped cache, got DisableFor %#v", options.Client.Cache.DisableFor)
	}

	if countObjectGVK(t, options.Scheme, options.Client.Cache.DisableFor, &rbacv1.ClusterRole{}) != 0 {
		t.Fatalf("expected webhook runtime ClusterRoles to use the scoped cache, got DisableFor %#v", options.Client.Cache.DisableFor)
	}
}
...
func assertNamespaceCacheNames(t *testing.T, namespaces map[string]cache.Config, expected []string) {
	t.Helper()

	if namespaces == nil {
		t.Fatal("expected namespace cache map, got nil")
	}

	if len(namespaces) != len(expected) {
		t.Fatalf("expected namespace cache entries %v, got %#v", expected, namespaces)
	}

	for _, namespace := range expected {
		if _, ok := namespaces[namespace]; !ok {
			t.Fatalf("expected namespace cache entry %q in %#v", namespace, namespaces)
		}
	}
}
