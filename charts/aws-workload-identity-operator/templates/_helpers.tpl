...
{{/*
Manager argument for ClusterProfile cache namespace scope.
*/}}
{{- define "awio.clusterProfileNamespacesArg" -}}
{{- $namespaces := default (list) .Values.clusterInventory.clusterProfileNamespaces | uniq -}}
{{- if not $namespaces -}}
{{- $namespaces = list (include "awio.namespace" .) -}}
{{- end -}}
{{- if and (has "*" $namespaces) (gt (len $namespaces) 1) -}}
{{- fail "clusterInventory.clusterProfileNamespaces must be either [\"*\"] for all namespaces or a list of explicit namespaces, not both" -}}
{{- end -}}
{{- printf "--cluster-profile-namespaces=%s" (join "," $namespaces) -}}
{{- end -}}
