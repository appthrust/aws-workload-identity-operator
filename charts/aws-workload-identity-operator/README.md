...
The remote permissions ManifestWork creates the webhook namespace and
least-privilege bootstrap RBAC needed by the operator's current remote
controllers. It does not grant `cluster-admin`; some cluster-wide `list/watch`
permissions are still required by the remote caches.
...
For live clusters, create and wait for the new MSA credential first, then roll
the operator with the matching Cluster Inventory provider arg, and remove the
old MSA/RBAC only after remote access is healthy and workload identity
