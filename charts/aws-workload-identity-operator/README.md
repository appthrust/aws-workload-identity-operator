...
The chart installs only the operator and platform access plumbing. Per-cluster
`AWSWorkloadIdentityConfig` objects choose `SelfHostedIRSA`, `EKSIRSA`, or
`EKSPodIdentity`. For `EKSIRSA`, the config may ask the operator to manage the
IAM OIDC provider through ACK IAM or to reference an externally managed
provider ARN.

## Install

Chart version `0.1.0` is published with the `v0.1.0` release. Install the
chart from GHCR's OCI registry:
...
`clusterInventory.clusterProfileNamespaces` controls where the manager watches
and caches `ClusterProfile` objects. Empty means the operator install
namespace, which is the default; the chart renders the effective release
namespace into `--cluster-profile-namespaces`. Set explicit namespaces when
your cluster manager publishes target `ClusterProfile` objects elsewhere. Use
`["*"]` only when the operator must watch every namespace; `["*"]` cannot be
mixed with explicit namespaces.
...
Hosted controller Pods that use the chart-created OCM managed service account
can mount the remote IRSA tools image and call
`aws-remote-irsa-credential-process` with the chart-generated provider file
through AWS `credential_process`.
See [Hub-Side Remote IRSA Consumers](../../docs/guides/remote-irsa-consumers.md#hosted-controller-image-volume-example).
...
`0.1.0` tag points at the manager image published with the `v0.1.0` release.
...
Review [Upgrades](../../docs/operations/upgrades.md) before rolling back live
clusters or making disruptive OCM access-provider changes.
...
`scheme`, `honorLabels`, `relabelings`, `metricRelabelings`,
`namespaceSelector`, `bearerTokenFile`, and `authorization` are supported.

The operator's `/metrics` endpoint is gated by controller-runtime's
`filters.WithAuthenticationAndAuthorization` (see `cmd/manager/main.go`), so
anonymous scrapes are rejected with `401 Unauthorized`. The chart therefore
defaults `serviceMonitor.bearerTokenFile` to
`/var/run/secrets/kubernetes.io/serviceaccount/token`, which mounts the
scraping Prometheus Pod's own ServiceAccount token — the canonical pattern
for in-cluster Prometheus (and the kube-prometheus-stack default). To use a
Secret-backed token instead, set `serviceMonitor.authorization` (a
Prometheus Operator `authorization` block); it overrides `bearerTokenFile`,
matching the CRD's mutual-exclusion rule. The scraper's ServiceAccount must
still hold a ClusterRole granting
...
For `SelfHostedIRSA`, the operator manager also verifies, writes, and deletes
the `.well-known/openid-configuration` and `keys.json` S3 objects with the S3
API.
