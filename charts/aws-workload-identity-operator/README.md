...
> **Status:** Pre-release. The OCI chart and image below are not published yet.
> Until tagpr cuts `v0.1.0`, install from a local checkout
> (`./charts/aws-workload-identity-operator`). The `0.1.0` / `v0.1.0` strings
> describe the targeted first release, not a pointer to a published artifact.

After the first release ships, install the chart from GHCR's OCI registry:
...
a source checkout (this is the only supported install path until `v0.1.0`).
