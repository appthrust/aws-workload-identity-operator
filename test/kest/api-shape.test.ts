...
describe("manifest shape", () => {
  test("manager uses image volumes for ClusterProfile access plugin", async () => {
    ...
    expect(manifest).toContain("--cluster-profile-namespaces=system");
    expect(manifest).not.toContain("POD_NAMESPACE");
  ...
  test("chart CRDs stay in sync with config CRDs", async () => {
    for (const name of [
      ...
      expect(readFileSync(`charts/aws-workload-identity-operator/crds/${name}`, "utf8")).toBe(
  ...
  test("chart validates values through values.schema.json", async () => {
    ...
    expect(schema.properties.clusterInventory.required).toContain("clusterProfileNamespaces");
    expect(schema.properties.clusterInventory.required).toContain("accessProviders");
    expect(schema.properties.clusterInventory.properties.clusterProfileNamespaces.uniqueItems).toBe(true);
    expect(schema.properties.clusterInventory.properties.clusterProfileNamespaces.items.anyOf[0].$ref).toBe(
      "#/definitions/dnsLabel",
    );
    expect(schema.properties.clusterInventory.properties.clusterProfileNamespaces.items.anyOf[1].const).toBe("*");
    expect(schema.properties.clusterInventory.allOf[0].then.properties.clusterProfileNamespaces.maxItems).toBe(1);
    ...
    expect(helpers).toContain("clusterProfileNamespaces");
  });
...
`);
  ...
  test("chart renders ClusterProfile namespace watch scope", async () => {
    const manifest = helmTemplate();
    expect(manifest).not.toContain("name: POD_NAMESPACE");
    expect(manifest).toContain("--cluster-profile-namespaces=awio-system");

    const namespaceOverrideManifest = helmTemplate(`
namespaceOverride: operator-ns
`);
    expect(namespaceOverrideManifest).toContain("--cluster-profile-namespaces=operator-ns");

    const scopedManifest = helmTemplate(`
clusterInventory:
  clusterProfileNamespaces:
    - team-a
    - team-b
`);
    expect(scopedManifest).toContain("--cluster-profile-namespaces=team-a,team-b");

    const allNamespacesManifest = helmTemplate(`
clusterInventory:
  clusterProfileNamespaces:
    - "*"
`);
    expect(allNamespacesManifest).toContain("--cluster-profile-namespaces=*");

    const failure = helmTemplateFailure(`
clusterInventory:
  clusterProfileNamespaces:
    - "*"
    - team-a
`);
    expect(failure).toContain("clusterProfileNamespaces");

    const emptyEntryFailure = helmTemplateFailure(`
clusterInventory:
  clusterProfileNamespaces:
    - ""
`);
    expect(emptyEntryFailure).toContain("clusterProfileNamespaces");
  });
