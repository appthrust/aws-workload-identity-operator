...
make helm-template
make verify-static
make verify-version-sync
make test-version-sync
make test-kest
```
...
`make verify-version-sync` runs `hack/sync-chart-version.sh --check` to confirm
that every prose anchor in the Helm chart README, install guide, compatibility
reference, and remote-IRSA consumer/OCM sidecar guides matches the current
chart version in `charts/aws-workload-identity-operator/Chart.yaml`. It proves
idempotency at the current version but cannot detect a renderer pattern that
happens to be a no-op for that version. `make test-version-sync` runs
`hack/test-sync-chart-version.sh`, which mirrors the managed files into a
tmpdir, bumps the chart version to a synthetic pre-release identifier, and
asserts that every documented prose anchor and image coordinate is rewritten
end-to-end. Run both before changing any renderer pattern, any prose anchor in
the managed docs, or `charts/aws-workload-identity-operator/Chart.yaml`.
...
## Remote IRSA Helper Binaries and Images

The remote IRSA path ships two helper binaries built from this module:

- `cmd/aws-remote-irsa-credential-process` — AWS CLI/SDK
  `credential_process` plugin that resolves remote IRSA credentials.
- `cmd/aws-irsa-sidecar` — sidecar that projects remote IRSA tokens for
  workloads.

Build both helper binaries locally to `bin/`:

```sh
make build-remote-irsa-tools
```

Build only the sidecar binary:

```sh
make build-aws-irsa-sidecar
```

Build the helper container images locally using the `Dockerfile` targets:

```sh
make docker-build-remote-irsa-tools \
    REMOTE_IRSA_TOOLS_IMAGE=ghcr.io/appthrust/aws-workload-identity-operator/remote-irsa-tools:dev

make docker-build-aws-irsa-sidecar \
    AWS_IRSA_SIDECAR_IMAGE=ghcr.io/appthrust/aws-workload-identity-operator/aws-irsa-sidecar:dev
```

`docker-build-remote-irsa-tools` builds the `remote-irsa-tools` stage and
`docker-build-aws-irsa-sidecar` builds the `aws-irsa-sidecar` stage of the
multi-target `Dockerfile`. Override `REMOTE_IRSA_TOOLS_IMAGE` or
`AWS_IRSA_SIDECAR_IMAGE` to push to a different registry or tag while
iterating locally.

## Release Automation

Merges to `main` run tagpr. When tagpr creates a version tag, the release
workflow publishes the multi-arch manager image to
`ghcr.io/<owner>/aws-workload-identity-operator`, the multi-arch helper images
to `ghcr.io/<owner>/aws-workload-identity-operator/remote-irsa-tools` and
`ghcr.io/<owner>/aws-workload-identity-operator/aws-irsa-sidecar`, and pushes
the Helm chart to
`oci://ghcr.io/<owner>/helm-charts/aws-workload-identity-operator`. All images
and the chart are signed with cosign. The chart version and app version are
derived from the release tag without the leading `v`.
