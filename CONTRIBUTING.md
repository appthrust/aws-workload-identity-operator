...
GitHub Actions publishes `ghcr.io/<owner>/aws-workload-identity-operator` on
pushes to `main` and version tags. Pull requests build the image without
pushing. The image build uses released Go modules only; workflows should not
...
runs an AWS CLI canary Job through the remote pod identity webhook. It requires
a local Docker unix socket; set `E2E_DOCKER_SOCK` when the socket cannot be
inferred from `DOCKER_HOST` or `/var/run/docker.sock`. It cleans up Kubernetes
and AWS resources by default. Set `E2E_KEEP_HUB=1` or
`E2E_KEEP_WORKLOAD_CLUSTER=1` to keep clusters after cleanup, or
`E2E_SKIP_CLEANUP=1` only when you intentionally want to inspect all resources
manually. Generated hub and workload cluster
contexts are merged into the first path in `KUBECONFIG`, or `~/.kube/config`
when `KUBECONFIG` is unset, so tools such as k9s can switch to them while the
e2e is running. Set `E2E_KUBECONFIG_MERGE_TARGET` to override the merge
destination. Omit `AWS_PROFILE` when using environment-provided AWS credentials
or the AWS CLI's default credential chain.
