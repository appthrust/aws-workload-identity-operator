...
controller boundaries, AWS resource model, security constraints, and validation
model.

## Prerequisites

- Kubernetes 1.33 or newer. The Helm chart uses Kubernetes image volumes for
...
- OCM `managed-serviceaccount` with
  `featureGates.clusterProfileCredSyncer=true`, plus a per-managed-cluster
  `ManagedServiceAccount` named `aws-workload-identity-operator` labelled
...
endpoints also require `aws.allowUnsafeEndpointURLs=true`. S3 endpoints that
require path-style addressing can use `s3.usePathStyle=true`. This mirrors
ACK's AWS API endpoint override; it does not change the public
...
The default Cluster Inventory access provider is OCM `cp-creds`, mounted as an
image volume:
...
The workload namespace is the target-cluster boundary. For example, this
`ClusterProfile` describes target cluster `wlc-a`. Its status is normally
written by Cluster Inventory or OCM:
...
metadata:
  name: default
  # Same as ClusterProfile metadata.name in the direct namespace/name model.
  # For OCM-generated ClusterProfiles, use the
  # open-cluster-management.io/cluster-name label value instead.
  namespace: wlc-a
spec:
  type: SelfHostedIRSA
