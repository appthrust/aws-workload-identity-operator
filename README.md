...
> **Status:** Pre-release. The OCI chart and image referenced from the install
> docs are not published yet. Until tagpr cuts `v0.1.0`, install from a local
> checkout (`./charts/aws-workload-identity-operator`). The `0.1.0` / `v0.1.0`
> strings in the docs describe the targeted first release, not a pointer to a
> published artifact.

Install the operator from a local chart checkout today, or with the future
published Helm chart once the first release ships. The command and chart setup
details live in [Install With Helm](docs/guides/install-helm.md); full chart
