...
func buildConfigFromClusterProfile(accessConfig *access.Config, profile *clusterinventoryv1alpha1.ClusterProfile) (*rest.Config, error) {
	...
	cfg, err := cloneAccessConfig(accessConfig).BuildConfigFromCP(profile)
	if err != nil {
...
func restImpersonationEqual(a, b rest.ImpersonationConfig) bool {
	return a.UserName == b.UserName &&
		...
		stringSlicesMapEqual(a.Extra, b.Extra)
}
...
func stringSlicesMapEqual(a, b map[string][]string) bool {
	if len(a) != len(b) {
		return false
	}
	...
	for key, aValues := range a {
		bValues, ok := b[key]
		if !ok || !slices.Equal(aValues, bValues) {
			return false
		}
	}
