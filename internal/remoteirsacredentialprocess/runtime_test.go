...
func TestRunWritesCredentialProcessJSON(t *testing.T) {
	...
	code := Run(context.Background(), testRuntimeOptions(t), &stdout, &stderr, deps)
	if code != 0 {
...
func TestSanitizeErrorRedactsSecretBearingErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt, func(t *testing.T) {
			if got := SanitizeError(errors.New(tt)); got != "remote IRSA credential_process failed" {
...
func TestRunReportsMissingDependency(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run(context.Background(), testRuntimeOptions(t), &stdout, &stderr, Dependencies{})
	if code != 1 {
		t.Fatalf("exit code = %d, want 1", code)
	}
	if stdout.Len() != 0 {
