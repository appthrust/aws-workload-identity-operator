...
func testIAMPolicyDocument(raw string) *runtime.RawExtension {
...
func TestPolicyDocumentStringCompactsJSONObject(t *testing.T) {
	doc := testIAMPolicyDocument(`{
		"Version": "2012-10-17",
		"Statement": [
			{
				"Effect": "Allow",
	...
	got, err := PolicyDocumentString(doc)
	if err != nil {
		t.Fatalf("expected policy document to serialize, got %v", err)
	}

	want := `{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Action":["s3:GetObject"],"Resource":"*"}]}`
	if got != want {
		t.Fatalf("expected compact JSON %s, got %s", want, got)
	}
}

func TestPolicyDocumentStringRejectsOversizedDocument(t *testing.T) {
	doc := testIAMPolicyDocument(`{"Statement":[{"Effect":"Allow","Action":"s3:GetObject","Resource":"` + strings.Repeat("a", 6144) + `"}]}`)

	if _, err := PolicyDocumentString(doc); err == nil || !strings.Contains(err.Error(), "6144") {
		t.Fatalf("expected 6144 character limit error, got %v", err)
	}
}
