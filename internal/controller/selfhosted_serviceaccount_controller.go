...
func patchRemoteServiceAccountObjectAnnotations(ctx context.Context, c client.Client, sa *corev1.ServiceAccount, roleARN string) (controllerutil.OperationResult, error) {
	...
	// Optimistic lock: a non-optimistic MergeFrom would let a concurrent write of
	// a newer role ARN between the Get above and this Patch be silently
	// overwritten. With resourceVersion in the patch, a stale write loses with
	// 409 and the level-triggered reconciler re-reads the latest state.
	if err := c.Patch(ctx, sa, client.MergeFromWithOptions(base, client.MergeFromWithOptimisticLock{})); err != nil {
...
func removeRemoteServiceAccountAnnotations(ctx context.Context, c client.Client, subject identityv1.ServiceAccountSubject, roleARN string) (controllerutil.OperationResult, error) {
	...
	// Optimistic lock: the in-process guard above is a snapshot from Get time. A
	// newer role ARN written between Get and this Patch would otherwise be wiped
	// by the unconditional annotation delete. resourceVersion turns the conflict
	// into a 409, the reconciler re-Gets, and the guard then skips correctly.
	if err := c.Patch(ctx, sa, client.MergeFromWithOptions(base, client.MergeFromWithOptimisticLock{})); err != nil {
		return controllerutil.OperationResultNone, fmt.Errorf("patch remote ServiceAccount %s/%s: %w", sa.Namespace, sa.Name, err)
	}
