...
func (r *SelfHostedServiceAccountReconciler) reconcileRemoteServiceAccount(ctx context.Context, log logr.Logger, remoteClient client.Client, deliveryType identityv1.DeliveryType, inventoryNamespace string, key types.NamespacedName) error {
	...
	// The indexer order is non-deterministic, so picking any one Role's RoleARN
	// would be unstable across reconciles. Surface a Warning on the remote SA and
	// skip the patch; once the conflict is resolved (one Role left, or the
	// duplicates are deleted), the next reconcile delivers the correct annotation.
	if len(activeRoles) > 1 {
		recordServiceAccountMultiRoleConflict(r.Recorder, current, conflictingRoleNames(activeRoles))
