...
func logRemoteApply(log logr.Logger, resource, name string, operation controllerutil.OperationResult, err error) {
	...
	logApplyOutcome(log.WithValues(logKeyDeliveryType, deliveryType), "remote", resource, name, operation, err)
}
...
func recordAnnotationRepaired(recorder events.EventRecorder, obj client.Object) {
	recordNormalEvent(recorder, obj, identityv1.ReasonAnnotationRepaired, eventActionRepairAnnotation, "repaired remote ServiceAccount annotations")
}
