...
const (
	...
	logKeyConflictingRoles    = "awio.conflicting_roles"
...
// Log messages referenced from tests must use these constants so the test
// assertion stays in lock-step with the emit site.
const (
	logMsgSkipSARepairMultiRole = "skipping ServiceAccount annotation repair: multiple active AWSServiceAccountRole objects bind the target ServiceAccount"
)
...
func eventIsACKWaiting(condition *metav1.Condition) bool {
	...
	return condition.Reason == identityv1.ReasonWaitingForACK ||
		...
		condition.Reason == identityv1.ReasonACKResourceIdentifierPending
}
