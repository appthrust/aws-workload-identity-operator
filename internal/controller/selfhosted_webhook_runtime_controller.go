...
//
// apiReader is the uncached remote Reader and is used by every ensure* helper
// to decide adoption against the live API server state, because the cached
// remoteClient is selector-scoped to the operator's managed-by/runtime labels
// (see SelfHostedWebhookRuntimeCacheByObject). Without the uncached probe,
// pre-existing unlabeled objects from upstream installs are invisible to the
// cache and would slip past the adoption guard.
func applyRemoteWebhookRuntime(ctx context.Context, log logr.Logger, apiReader client.Reader, remoteClient client.Client, webhookNamespace, image string) (webhookRuntimeOutcome, error) {
	if webhookNamespace == "" {
	...
	op, err := ensureRemoteNamespace(ctx, apiReader, remoteClient, webhookNamespace)

	if isErrRemoteNamespaceBootstrapPending(err) {
		// Bootstrap-pending is a benign level-triggered precondition surfaced
		// via WebhookRuntimeReady; skip logRemoteApply so the steady requeue
		// cadence does not emit a top-level Error log nor inflate the
		// apply-failed remote-delivery counter against a state that has a
		// dedicated condition reason.
		log.V(1).Info("remote webhook namespace bootstrap pending", "namespace", webhookNamespace)

		outcome.Condition = webhookRuntimeCondition{
			Ready:   false,
			Reason:  identityv1.ReasonRemoteNamespaceBootstrapPending,
			Message: err.Error(),
		}
	...
	certObservation, caBundle, certFingerprint, err := applyRemoteWebhookCertificates(ctx, log, apiReader, remoteClient, webhookNamespace)
	if err != nil {
		return outcome, err
	}
	...
	if err := applyRemoteSupportingResources(ctx, log, apiReader, remoteClient, webhookNamespace); err != nil {
		return outcome, err
	}

	deployment, op, err := ensureRemoteDeployment(ctx, apiReader, remoteClient, webhookNamespace, image, certFingerprint)
	...
	condition, err := finishRemoteWebhookAdmission(ctx, log, apiReader, remoteClient, webhookNamespace, caBundle, deployment)
	outcome.Condition = condition

	return outcome, err
}

func applyRemoteWebhookCertificates(ctx context.Context, log logr.Logger, apiReader client.Reader, remoteClient client.Client, webhookNamespace string) (webhookRuntimeObservation, []byte, string, error) {
	caOp, caBundle, caCert, caKey, err := ensureRemoteCASecret(ctx, apiReader, remoteClient, webhookNamespace)
	...
	servingOp, certNotAfter, certFingerprint, err := ensureRemoteServingTLSSecret(ctx, apiReader, remoteClient, webhookNamespace, caCert, caKey)
...
func finishRemoteWebhookAdmission(ctx context.Context, log logr.Logger, apiReader client.Reader, remoteClient client.Client, webhookNamespace string, caBundle []byte, deployment *appsv1.Deployment) (webhookRuntimeCondition, error) {
	...
	if !readiness.Ready {
		...
		// before that fetch runs. apiReader (uncached) is required for the same reason
		// as in createOrUpdateAdoptableWebhookRuntime: the remote cache is
		// selector-scoped to the managed-by/runtime labels
		// (SelfHostedWebhookRuntimeCacheByObject), so a pre-existing unlabeled MWHC is
		// invisible to the cached client. Going through the cache here would
		// short-circuit as "waiting for Deployment" and silently mask the
		// errUnmanagedRemoteRuntime that ensureRemoteMutatingWebhookConfiguration
		// would otherwise surface — the same contract as the deletion-path probe in
		// deleteManagedRemoteWebhookRuntimeObject.
		mwhcExists, err := remoteMutatingWebhookConfigurationExists(ctx, apiReader)
		if err != nil {
	...
	webhook, op, err := ensureRemoteMutatingWebhookConfiguration(ctx, apiReader, remoteClient, webhookNamespace, caBundle)
...
func applyRemoteSupportingResources(ctx context.Context, log logr.Logger, apiReader client.Reader, remoteClient client.Client, webhookNamespace string) error {
	parallel := []struct {
		kind  string
		apply func(context.Context, client.Reader, client.Client, string) (controllerutil.OperationResult, error)
	}{
	...
	for _, step := range parallel {
		g.Go(func() error {
			op, err := step.apply(gCtx, apiReader, remoteClient, webhookNamespace)
...
func webhookRuntimeUpdateNeedsReconcile(oldObj, newObj client.Object) bool {
	...
	// status writes do not bump Generation; assessRemoteDeploymentReadiness consumes these fields.
	if oldDeployment.Status.ObservedGeneration != newDeployment.Status.ObservedGeneration ||
		...
		deploymentAvailableConditionStatus(oldDeployment) != deploymentAvailableConditionStatus(newDeployment) {
		return true
	}
...
func deploymentAvailableConditionStatus(d *appsv1.Deployment) corev1.ConditionStatus {
...
func isManagedWebhookRuntimeObject(obj client.Object) bool {
	...
	return labels[identityv1.LabelRuntime] == identityv1.RuntimeWebhook
}

// adoptableForApply guards against silently overwriting a pre-existing object
// the operator did not create — most commonly the upstream
// amazon-eks-pod-identity-webhook chart's install. New objects (empty
// ResourceVersion) and operator-labeled objects are safe to mutate.
func adoptableForApply(obj client.Object) bool {
	return obj.GetResourceVersion() == "" || isManagedWebhookRuntimeObject(obj)
}

func errApplyUnmanagedRemoteRuntime(obj client.Object) error {
	return fmt.Errorf("%w: refusing to apply %T %q: pre-existing object lacks %s=%s label; remove the object or add the operator labels to permit reconcile, or set %s annotation on the AWSWorkloadIdentityConfig and delete the Config to abandon this install via force-delete and leave this object behind",
...
// createOrUpdateAdoptableWebhookRuntime wraps createOrUpdate with the
// apply-side adoption guard so every webhook-runtime ensure* helper applies
// the same refuse-to-overwrite-unmanaged contract without restating it.
//
// The ownership probe is routed through apiReader (uncached) because the
// remote cluster's cache is selector-scoped to objects bearing the operator's
// managed-by/runtime labels (see SelfHostedWebhookRuntimeCacheByObject). A
// cached Get would silently filter out pre-existing unlabeled objects and
// let adoptableForApply pass a false-positive (empty ResourceVersion), so the
// subsequent Create would collide with the real object on the API server and
// surface as a generic conflict rather than the intended
// errUnmanagedRemoteRuntime sentinel. Mirrors the deletion-path pattern in
// deleteManagedRemoteWebhookRuntimeObject. The in-mutate adoptableForApply
// check remains as defence-in-depth against label drift between the probe
// and the cached Get inside controllerutil.CreateOrUpdate.
func createOrUpdateAdoptableWebhookRuntime(ctx context.Context, apiReader client.Reader, c client.Client, obj client.Object, mutate controllerutil.MutateFn) (controllerutil.OperationResult, error) {
	probe, ok := obj.DeepCopyObject().(client.Object)
	if !ok {
		return controllerutil.OperationResultNone, fmt.Errorf("deep copy of %T did not yield a client.Object", obj)
	}

	switch err := apiReader.Get(ctx, client.ObjectKeyFromObject(obj), probe); {
	case apierrors.IsNotFound(err):
		// Object is absent from the live API server; safe to Create via the
		// cached client.
	case err != nil:
		return controllerutil.OperationResultNone, fmt.Errorf("probe %T %s/%s for adoption: %w", obj, obj.GetNamespace(), obj.GetName(), err)
	case !isManagedWebhookRuntimeObject(probe):
		return controllerutil.OperationResultNone, errApplyUnmanagedRemoteRuntime(obj)
	}

	return createOrUpdate(ctx, c, nil, nil, obj, func() error {
		if !adoptableForApply(obj) {
			return errApplyUnmanagedRemoteRuntime(obj)
		}
...
func ensureRemoteNamespace(ctx context.Context, apiReader client.Reader, c client.Client, name string) (controllerutil.OperationResult, error) {
	// Probe the live API server before issuing a write; the selector-scoped
	// remote cache would otherwise hide a pre-existing user-owned namespace,
	// causing the cached Get inside controllerutil.CreateOrUpdate to return
	// NotFound and the subsequent Create to collide with AlreadyExists. When
	// the namespace exists and is unmanaged, leave it untouched.
	probe := &corev1.Namespace{}
	switch err := apiReader.Get(ctx, client.ObjectKey{Name: name}, probe); {
	case apierrors.IsNotFound(err):
		// Absence is treated as a level-triggered precondition: the
		// chart-managed OCM ManifestWork owns namespace bootstrap, and the
		// delegated remote ServiceAccount intentionally has no cluster-wide
		// namespaces:create verb, so the controller surfaces a Condition and
		// waits rather than issuing a Create that would fail Forbidden.
		return controllerutil.OperationResultNone, fmt.Errorf("%w: namespace %q absent on workload cluster; expected chart-managed OCM ManifestWork (or equivalent external bootstrap) to create it", errRemoteNamespaceBootstrapPending, name)
	case err != nil:
		return controllerutil.OperationResultNone, fmt.Errorf("probe Namespace %q: %w", name, err)
	case !isManagedWebhookRuntimeObject(probe):
...
func ensureRemoteCASecret(ctx context.Context, apiReader client.Reader, c client.Client, namespace string) (controllerutil.OperationResult, []byte, *x509.Certificate, *rsa.PrivateKey, error) {
	...
	op, err := createOrUpdateAdoptableWebhookRuntime(ctx, apiReader, c, secret, func() error {
...
func ensureRemoteServingTLSSecret(ctx context.Context, apiReader client.Reader, c client.Client, namespace string, caCert *x509.Certificate, caKey *rsa.PrivateKey) (controllerutil.OperationResult, metav1.Time, string, error) {
...
func ensureRemoteServiceAccount(ctx context.Context, apiReader client.Reader, c client.Client, namespace string) (controllerutil.OperationResult, error) {
	sa := &corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: webhookComponentName, Namespace: namespace}}

	return createOrUpdateAdoptableWebhookRuntime(ctx, apiReader, c, sa, func() error {
...
func ensureRemoteClusterRole(ctx context.Context, apiReader client.Reader, c client.Client, _ string) (controllerutil.OperationResult, error) {
	role := &rbacv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: webhookComponentName}}

	return createOrUpdateAdoptableWebhookRuntime(ctx, apiReader, c, role, func() error {
...
func ensureRemoteClusterRoleBinding(ctx context.Context, apiReader client.Reader, c client.Client, namespace string) (controllerutil.OperationResult, error) {
	binding := &rbacv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: webhookComponentName}}

	return createOrUpdateAdoptableWebhookRuntime(ctx, apiReader, c, binding, func() error {
...
func ensureRemoteService(ctx context.Context, apiReader client.Reader, c client.Client, namespace string) (controllerutil.OperationResult, error) {
	service := &corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: webhookComponentName, Namespace: namespace}}

	return createOrUpdateAdoptableWebhookRuntime(ctx, apiReader, c, service, func() error {
...
func ensureRemoteMutatingWebhookConfiguration(ctx context.Context, apiReader client.Reader, c client.Client, namespace string, caBundle []byte) (*admissionregistrationv1.MutatingWebhookConfiguration, controllerutil.OperationResult, error) {
	path := "/mutate"
	...
	op, err := createOrUpdateAdoptableWebhookRuntime(ctx, apiReader, c, webhook, func() error {
...
func ensureRemoteDeployment(ctx context.Context, apiReader client.Reader, c client.Client, namespace, image, certFingerprint string) (*appsv1.Deployment, controllerutil.OperationResult, error) {
	...
	op, err := createOrUpdateAdoptableWebhookRuntime(ctx, apiReader, c, deployment, func() error {
		...
		deployment.Spec.Template.Labels = mergeStringMap(deployment.Spec.Template.Labels, webhookDeploymentPodLabels())
...
func setWebhookRuntimeLabels(obj client.Object) {
	obj.SetLabels(mergeStringMap(obj.GetLabels(), webhookRuntimeLabels()))
}

func remoteMutatingWebhookConfigurationExists(ctx context.Context, c client.Reader) (bool, error) {
	webhook := &admissionregistrationv1.MutatingWebhookConfiguration{ObjectMeta: metav1.ObjectMeta{Name: webhookComponentName}}
...
func deleteRemoteWebhookRuntime(ctx context.Context, apiReader client.Reader, c client.Client, namespace string) error {
	if namespace == "" {
	...
	if err := deleteManagedRemoteWebhookRuntimeObject(ctx, apiReader, c, mwhc); err != nil {
		return err
	}
	...
	for _, obj := range parallel {
		g.Go(func() error {
			return deleteManagedRemoteWebhookRuntimeObject(gCtx, apiReader, c, obj)
		})
	}
...
// errRemoteNamespaceBootstrapPending sentinel signals that the remote webhook
// namespace has not yet been bootstrapped on the workload cluster. The
// chart-managed OCM ManifestWork creates the namespace; granting the delegated
// ServiceAccount cluster-wide namespaces:create would broaden its RBAC beyond
// least privilege, so the controller treats absence as a level-triggered
// precondition rather than attempting to create it.
var errRemoteNamespaceBootstrapPending = errors.New("remote webhook namespace has not been bootstrapped")

func isErrRemoteNamespaceBootstrapPending(err error) bool {
	return errors.Is(err, errRemoteNamespaceBootstrapPending)
}

func deleteManagedRemoteWebhookRuntimeObject(ctx context.Context, apiReader client.Reader, c client.Client, obj client.Object) error {
	log := logf.FromContext(ctx)

	// Use the uncached APIReader so a selector-scoped remote cache cannot hide
	// pre-existing or label-drifted objects from the ownership check below.
	if err := apiReader.Get(ctx, client.ObjectKeyFromObject(obj), obj); err != nil {
		if apierrors.IsNotFound(err) {
			return nil
		}
