...
const (
	...
	DefaultPodIdentityWebhookImage = "public.ecr.aws/eks/amazon-eks-pod-identity-webhook:v0.6.14"
...
type webhookRuntimeOwner struct {
	ConfigUID          string
...
func applyRemoteWebhookRuntime(ctx context.Context, log logr.Logger, remoteClient client.Client, owner webhookRuntimeOwner, webhookNamespace, image string) (webhookRuntimeOutcome, error) {
	if webhookNamespace == "" {
	...
	op, err := ensureRemoteNamespace(ctx, remoteClient, webhookNamespace, owner)
	...
	certObservation, caBundle, certFingerprint, err := applyRemoteWebhookCertificates(ctx, log, remoteClient, webhookNamespace, owner)
	if err != nil {
		return outcome, err
	}
	...
	if err := applyRemoteSupportingResources(ctx, log, remoteClient, webhookNamespace, owner); err != nil {
		return outcome, err
	}

	deployment, op, err := ensureRemoteDeployment(ctx, remoteClient, webhookNamespace, image, owner, certFingerprint)
	...
	condition, err := finishRemoteWebhookAdmission(ctx, log, remoteClient, webhookNamespace, owner, caBundle, deployment)
	outcome.Condition = condition

	return outcome, err
}

func applyRemoteWebhookCertificates(ctx context.Context, log logr.Logger, remoteClient client.Client, webhookNamespace string, owner webhookRuntimeOwner) (webhookRuntimeObservation, []byte, string, error) {
	caOp, caBundle, caCert, caKey, err := ensureRemoteCASecret(ctx, remoteClient, webhookNamespace, owner)
	...
	servingOp, certNotAfter, certFingerprint, err := ensureRemoteServingTLSSecret(ctx, remoteClient, webhookNamespace, owner, caCert, caKey)
...
func finishRemoteWebhookAdmission(ctx context.Context, log logr.Logger, remoteClient client.Client, webhookNamespace string, owner webhookRuntimeOwner, caBundle []byte, deployment *appsv1.Deployment) (webhookRuntimeCondition, error) {
	...
	deploymentAvailable := remoteDeploymentAvailable(deployment)
	if !deploymentAvailable {
		...
		if !mwhcExists {
			return webhookRuntimeCondition{
				...
				Message: "waiting for remote webhook Deployment to become Available before installing admission configuration",
			}, nil
		}
	}

	webhook, op, err := ensureRemoteMutatingWebhookConfiguration(ctx, remoteClient, webhookNamespace, owner, caBundle)
	...
	if !deploymentAvailable {
		return webhookRuntimeCondition{
			Ready:   false,
			Reason:  identityv1.ReasonWaitingForWebhookDeployment,
			Message: "remote webhook Deployment is not Available",
		}, nil
	}
...
func applyRemoteSupportingResources(ctx context.Context, log logr.Logger, remoteClient client.Client, webhookNamespace string, owner webhookRuntimeOwner) error {
	parallel := []struct {
		kind  string
		apply func(context.Context, client.Client, string, webhookRuntimeOwner) (controllerutil.OperationResult, error)
	}{
		{kind: "ServiceAccount", apply: ensureRemoteServiceAccount},
		{kind: "Role", apply: ensureRemoteRole},
		{kind: "RoleBinding", apply: ensureRemoteRoleBinding},
	...
	for _, step := range parallel {
		g.Go(func() error {
			op, err := step.apply(gCtx, remoteClient, webhookNamespace, owner)
...
// SetupSelfHostedWebhookRuntimeController registers the remote runtime drift
// bridge. It watches only managed singleton runtime resources and enqueues the
// local Config owner through runtimeEvents.
func SetupSelfHostedWebhookRuntimeController(mcMgr mcmanager.Manager, localClient client.Client, runtimeEvents chan<- event.TypedGenericEvent[*identityv1.AWSWorkloadIdentityConfig]) error {
	...
	if err := builder.Complete(&SelfHostedWebhookRuntimeReconciler{
...
func ensureRemoteNamespace(ctx context.Context, c client.Client, name string, owner webhookRuntimeOwner) (controllerutil.OperationResult, error) {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: name}}

	return createOrUpdate(ctx, c, nil, nil, ns, func() error {
		setWebhookRuntimeLabels(ns, owner)

		return nil
	})
}

func ensureRemoteCASecret(ctx context.Context, c client.Client, namespace string, owner webhookRuntimeOwner) (controllerutil.OperationResult, []byte, *x509.Certificate, *rsa.PrivateKey, error) {
	...
	op, err := createOrUpdate(ctx, c, nil, nil, secret, func() error {
		setWebhookRuntimeLabels(secret, owner)
...
func ensureRemoteServingTLSSecret(ctx context.Context, c client.Client, namespace string, owner webhookRuntimeOwner, caCert *x509.Certificate, caKey *rsa.PrivateKey) (controllerutil.OperationResult, metav1.Time, string, error) {
...
func ensureRemoteServiceAccount(ctx context.Context, c client.Client, namespace string, owner webhookRuntimeOwner) (controllerutil.OperationResult, error) {
	...
	return createOrUpdate(ctx, c, nil, nil, sa, func() error {
		setWebhookRuntimeLabels(sa, owner)

		return nil
	})
}

func ensureRemoteRole(ctx context.Context, c client.Client, namespace string, owner webhookRuntimeOwner) (controllerutil.OperationResult, error) {
	role := &rbacv1.Role{ObjectMeta: metav1.ObjectMeta{Name: webhookComponentName, Namespace: namespace}}

	return createOrUpdate(ctx, c, nil, nil, role, func() error {
		setWebhookRuntimeLabels(role, owner)
		role.Rules = []rbacv1.PolicyRule{
			...
			},
			{
				APIGroups:     []string{""},
				Resources:     []string{"secrets"},
				ResourceNames: []string{webhookComponentName},
...
func ensureRemoteRoleBinding(ctx context.Context, c client.Client, namespace string, owner webhookRuntimeOwner) (controllerutil.OperationResult, error) {
	binding := &rbacv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: webhookComponentName, Namespace: namespace}}

	return createOrUpdate(ctx, c, nil, nil, binding, func() error {
		setWebhookRuntimeLabels(binding, owner)
		binding.Subjects = []rbacv1.Subject{{Kind: "ServiceAccount", Name: webhookComponentName, Namespace: namespace}}
		binding.RoleRef = rbacv1.RoleRef{APIGroup: "rbac.authorization.k8s.io", Kind: "Role", Name: webhookComponentName}

		return nil
	})
}

func ensureRemoteClusterRole(ctx context.Context, c client.Client, _ string, owner webhookRuntimeOwner) (controllerutil.OperationResult, error) {
...
func ensureRemoteClusterRoleBinding(ctx context.Context, c client.Client, namespace string, owner webhookRuntimeOwner) (controllerutil.OperationResult, error) {
...
func ensureRemoteService(ctx context.Context, c client.Client, namespace string, owner webhookRuntimeOwner) (controllerutil.OperationResult, error) {
	...
	return createOrUpdate(ctx, c, nil, nil, service, func() error {
		setWebhookRuntimeLabels(service, owner)
...
func ensureRemoteMutatingWebhookConfiguration(ctx context.Context, c client.Client, namespace string, owner webhookRuntimeOwner, caBundle []byte) (*admissionregistrationv1.MutatingWebhookConfiguration, controllerutil.OperationResult, error) {
	path := "/mutate"
	...
	// fails with AccessDenied far from the root cause. Per platform AGENTS.md,
	// "Ignore" is forbidden for the same reasoning that applies to validating
	// webhooks.
	...
	op, err := createOrUpdate(ctx, c, nil, nil, webhook, func() error {
		setWebhookRuntimeLabels(webhook, owner)
...
func ensureRemoteDeployment(ctx context.Context, c client.Client, namespace, image string, owner webhookRuntimeOwner, certFingerprint string) (*appsv1.Deployment, controllerutil.OperationResult, error) {
	...
	op, err := createOrUpdate(ctx, c, nil, nil, deployment, func() error {
		setWebhookRuntimeLabels(deployment, owner)
		...
		deployment.Spec.Template.Labels = mergeStringMap(deployment.Spec.Template.Labels, webhookDeploymentPodLabels(owner))
...
func webhookDeploymentPodLabels(owner webhookRuntimeOwner) map[string]string {
	labels := webhookRuntimeLabels(owner)
...
func desiredWebhookContainer(namespace, image string) corev1.Container {
	return corev1.Container{
		...
		Args: []string{
			...
			"--token-audience=" + identityaws.STSAudience,
		},
...
func webhookRuntimeOwnerForConfig(config *identityv1.AWSWorkloadIdentityConfig) webhookRuntimeOwner {
	return webhookRuntimeOwner{
		ConfigUID:          string(config.UID),
		InventoryNamespace: config.Namespace,
	}
}

func webhookRuntimeLabels(owner webhookRuntimeOwner) map[string]string {
	...
	if owner.ConfigUID != "" {
		labels[identityv1.LabelConfigUID] = owner.ConfigUID
	}

	if owner.InventoryNamespace != "" {
		labels[identityv1.LabelInventoryNS] = owner.InventoryNamespace
	}

	return labels
}

func setWebhookRuntimeLabels(obj client.Object, owner webhookRuntimeOwner) {
	mergeLabels(obj, webhookRuntimeLabels(owner))
}
...
func remoteDeploymentAvailable(deployment *appsv1.Deployment) bool {
	desired := int32(1)
...
func deleteRemoteWebhookRuntime(ctx context.Context, c client.Client, namespace string) error {
	...
	deletes := []client.Object{
		&admissionregistrationv1.MutatingWebhookConfiguration{ObjectMeta: metav1.ObjectMeta{Name: webhookComponentName}},
	...
	return deleteAllIgnoreMissing(ctx, c, deletes)
}
