...
func TestReconcileNormalEKSIRSAManagedCreatesProviderWithoutSelfHostedResources(t *testing.T) {
	...
	config := testEKSIRSAConfig(identityv1.OIDCProviderManagementManaged, "")
	providerARN := "arn:aws:iam::123456789012:oidc-provider/oidc.eks.ap-northeast-1.amazonaws.com/id/EXAMPLE"
	localClient := testConfigClient(
		t,
		config,
		testOperatorConfig(),
		testResolvedClusterProfile(config.Namespace),
		testOIDCProvider(config, true, providerARN),
	)
	reconciler := &AWSWorkloadIdentityConfigReconciler{
		Client:   localClient,
		Scheme:   testControllerScheme(t),
	...
	if stored.Status.BucketName != "" || stored.Status.PublishedKeyID != "" || stored.Status.WebhookRuntimeNamespace != "" || stored.Status.WebhookRuntimeCertNotAfter != nil {
...
func TestDeleteConfigChildrenHandlesEKSIRSAManagedAndExternal(t *testing.T) {
	...
	managedProvider := testOIDCProvider(managed, true, "arn:aws:iam::123456789012:oidc-provider/oidc.eks.ap-northeast-1.amazonaws.com/id/EXAMPLE")
	markOwnedByConfig(t, managed, managedProvider)
	externalARN := "arn:aws:iam::123456789012:oidc-provider/oidc.eks.ap-northeast-1.amazonaws.com/id/EXAMPLE"
	external := testEKSIRSAConfig(identityv1.OIDCProviderManagementExternal, externalARN)
	external.Namespace = "wlc-external"
	externalProvider := testOIDCProvider(external, true, externalARN)
...
func TestDeleteConfigChildrenHandlesEKSIRSAUnownedProvider(t *testing.T) {
	config := testEKSIRSAConfig(identityv1.OIDCProviderManagementManaged, "")
	provider := testOIDCProvider(config, true, "arn:aws:iam::123456789012:oidc-provider/oidc.eks.ap-northeast-1.amazonaws.com/id/EXAMPLE")
