...
func TestReconcileSelfHostedIssuerPreservesPreCreatedSigningSecretDataFromUncachedReader(t *testing.T) {
	...
	if !bytes.Equal(publisher.publicKeyPEM, publicKey) {
		t.Fatalf("expected publisher to use pre-created public key from uncached Secret")
	}

	if publisher.keyID != keyID {
		t.Fatalf("expected publisher key ID %q, got %q", keyID, publisher.keyID)
	}
...
func TestReconcileSelfHostedIssuerSkipsRepublishWhenIssuerHashUnchanged(t *testing.T) {
	ctx := context.Background()
	config := testSelfHostedConfig()
	publisher := &fakeOIDCIssuerPublisher{}
	reconciler := testConfigReconciler(t, publisher, config, testBucket(config, true))
	...
	if config.Status.PublishedIssuerHash == "" {
		t.Fatal("expected PublishedIssuerHash to be recorded after publish")
	}
	...
	if publisher.calls != 1 {
		t.Fatalf("expected second reconcile to skip publish when hash is unchanged, got %d", publisher.calls)
	}
}
...
func TestReconcileSelfHostedIssuerResetsPublishedSentinelsWhenBucketUnsynced(t *testing.T) {
	config := testSelfHostedConfig()
	config.Status.PublishedKeyID = "previous-key"
	config.Status.PublishedIssuerHash = "previous-hash"
	publisher := &fakeOIDCIssuerPublisher{}
	...
	if config.Status.PublishedIssuerHash != "" {
		t.Fatalf("expected PublishedIssuerHash to reset when bucket is not synced, got %q", config.Status.PublishedIssuerHash)
	}
}

// TestReconcileSelfHostedIssuerRepublishesWhenIssuerHashStale locks in the
// renderer-format-change scenario that the published-hash sentinel fixes: when
// the recorded fingerprint no longer matches the freshly rendered objects, the
// controller must re-publish even though the signing key ID is unchanged.
func TestReconcileSelfHostedIssuerRepublishesWhenIssuerHashStale(t *testing.T) {
	ctx := context.Background()
	config := testSelfHostedConfig()
	publisher := &fakeOIDCIssuerPublisher{}
	reconciler := testConfigReconciler(t, publisher, config, testBucket(config, true))
	...
	if config.Status.PublishedIssuerHash == "" {
		t.Fatal("expected PublishedIssuerHash to be recorded after first publish")
	}

	const stale = "stale-hash-from-prior-renderer-version"

	config.Status.PublishedIssuerHash = stale
	...
	if publisher.calls != 2 {
		t.Fatalf("expected second reconcile to re-publish when hash is stale, got %d", publisher.calls)
	}

	if config.Status.PublishedIssuerHash == stale {
		t.Fatalf("expected PublishedIssuerHash to be refreshed after re-publish, still %q", stale)
	}

	if config.Status.PublishedIssuerHash == "" {
		t.Fatal("expected PublishedIssuerHash to be recorded after re-publish")
	}
}

// TestReconcileSelfHostedIssuerSkipsRepublishWhenHashMatchesEvenWithLegacyPublishedKeyID
// asserts the hash gate alone is sufficient to skip re-publish: a config
// upgraded from a pre-hash operator version (PublishedKeyID empty) but with a
// pre-seeded matching PublishedIssuerHash must NOT trigger a re-publish.
func TestReconcileSelfHostedIssuerSkipsRepublishWhenHashMatchesEvenWithLegacyPublishedKeyID(t *testing.T) {
	ctx := context.Background()
	config := testSelfHostedConfig()
	publisher := &fakeOIDCIssuerPublisher{}
	reconciler := testConfigReconciler(t, publisher, config, testBucket(config, true))

	// First reconcile to materialize the signing Secret + capture the published
	// inputs (publicKeyPEM, keyID) from the fake publisher, so the test can
	// independently compute the expected hash without calling the production
	// helper under test.
	if err := reconciler.reconcileSelfHostedIssuer(ctx, config); err != nil {
		t.Fatal(err)
	}
	...
	issuerURL := config.Status.IssuerURL()
	keyID := publisher.keyID
	publicKeyPEM := bytes.Clone(publisher.publicKeyPEM)

	expectedHash := hashIssuerObjectsInline(t, issuerURL, publicKeyPEM, keyID)

	// Simulate a config that was reconciled by a pre-hash operator version: the
	// hash is correct (e.g. backfilled), but PublishedKeyID is empty.
	config.Status.PublishedIssuerHash = expectedHash
	...
	if publisher.calls != 0 {
		t.Fatalf("expected hash gate alone to skip publish, got %d publish calls", publisher.calls)
	}

	if config.Status.PublishedIssuerHash != expectedHash {
		t.Fatalf("expected PublishedIssuerHash to remain %q, got %q", expectedHash, config.Status.PublishedIssuerHash)
	}
}

// TestComputePublishedIssuerHashChangesWithIssuerInputs pins the fingerprint
// contract: any change to the renderer inputs (issuer URL, signing key + key
// ID) must produce a distinct hash so the controller re-publishes.
func TestComputePublishedIssuerHashChangesWithIssuerInputs(t *testing.T) {
	const (
		baselineIssuer  = "https://example-bucket.s3.ap-northeast-1.amazonaws.com"
		alternateIssuer = "https://example-bucket.s3.us-east-1.amazonaws.com"
	)

	_, publicKey1, keyID1, err := oidc.GenerateRSAKeyPEM(2048)
	if err != nil {
		t.Fatal(err)
	}

	_, publicKey2, keyID2, err := oidc.GenerateRSAKeyPEM(2048)
	if err != nil {
		t.Fatal(err)
	}

	baselineHash, err := computePublishedIssuerHash(baselineIssuer, publicKey1, keyID1)
	if err != nil {
		t.Fatalf("baseline hash: %v", err)
	}

	differentIssuerHash, err := computePublishedIssuerHash(alternateIssuer, publicKey1, keyID1)
	if err != nil {
		t.Fatalf("different-issuer hash: %v", err)
	}

	differentKeyHash, err := computePublishedIssuerHash(baselineIssuer, publicKey2, keyID2)
	if err != nil {
		t.Fatalf("different-key hash: %v", err)
	}
	...
	if baselineHash == differentIssuerHash {
		t.Fatalf("expected different issuer URL to change the hash, both = %q", baselineHash)
	}

	if baselineHash == differentKeyHash {
		t.Fatalf("expected different signing key+keyID to change the hash, both = %q", baselineHash)
	}

	if differentIssuerHash == differentKeyHash {
		t.Fatalf("expected the two perturbations to produce distinct hashes, both = %q", differentIssuerHash)
	}
}

// hashIssuerObjectsInline replicates the production fingerprint formula
// independently of computePublishedIssuerHash so the skip-gate test cannot
// trivially pass by coincidence. Keep this in sync with the formula in
// config_controller.go's computePublishedIssuerHash if the production helper
// changes — the resulting drift correctly forces a re-publish, which the
// stale-hash test already pins.
func hashIssuerObjectsInline(t *testing.T, issuerURL string, publicKeyPEM []byte, keyID string) string {
	t.Helper()

	objects, err := oidc.RenderIssuerObjects(issuerURL, publicKeyPEM, keyID)
	if err != nil {
		t.Fatalf("render OIDC issuer objects for inline hash: %v", err)
	}
...
// Regression for LC-04: when an OpenIDConnectProvider sharing the deterministic
...
// deleted by deleteEKSIRSAChildren so the LC-04 observability change does not
...
// Regression for DEL-01: when the IAM ACK OpenIDConnectProvider CRD is not
...
func assertManagedEKSIRSAConfigStatus(t *testing.T, stored *identityv1.AWSWorkloadIdentityConfig, providerARN string) {
	...
	if stored.Status.BucketName != "" ||
		stored.Status.PublishedKeyID != "" ||
		stored.Status.PublishedIssuerHash != "" ||
