...
func TestSelfHostedIssuerFailureClearsStaleReadyConditions(t *testing.T) {
	...
	// Simulate a successful prior reconcile at generation 1, then a spec change
	// (generation bumps to 2) that fails before the inner reconcile can refresh
	// any of the issuer-owned conditions. The helper must overwrite the stale
	// prior-generation True observations with current-generation failures so the
	// status does not lie about what has been verified this generation.
	config.Generation = 1
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIssuerReady, metav1.ConditionTrue, identityv1.ReasonReady, "ready")
	...
	issuer := meta.FindStatusCondition(config.Status.Conditions, identityv1.ConditionIssuerReady)
	if issuer == nil || issuer.Status != metav1.ConditionFalse ||
		issuer.ObservedGeneration != config.Generation ||
		issuer.Reason != identityv1.ReasonIssuerReconcileFailed {
		t.Fatalf("expected IssuerReady=False/IssuerReconcileFailed at generation %d, got %#v", config.Generation, issuer)
	}

	if !meta.IsStatusConditionFalse(config.Status.Conditions, identityv1.ConditionReady) {
		t.Fatalf("expected Ready=False to cascade from stale issuer failure: %#v", config.Status.Conditions)
	}
}

func TestSelfHostedIssuerFailureResetsStalePriorGenerationConditions(t *testing.T) {
	config := testSelfHostedConfig()
	issuerOwnedConditions := []string{
	...
	// Generation 1: a previous reconcile observed every issuer-owned sub-state
	// as Ready. Then the spec changes (Generation 2) and the inner reconcile
	// fails before it can refresh any of those observations.
	config.Generation = 1
	for _, condType := range issuerOwnedConditions {
		setCondition(&config.Status.Conditions, config.Generation, condType, metav1.ConditionTrue, identityv1.ReasonReady, "ready")
	}

	config.Generation = 2

	setSelfHostedIssuerFailureCondition(config, errors.New("signing secret unavailable"))

	for _, condType := range issuerOwnedConditions {
		assertConditionAtGeneration(t, config.Status.Conditions, condType, metav1.ConditionFalse, identityv1.ReasonIssuerReconcileFailed, config.Generation)
	}
}

func TestSelfHostedIssuerFailurePreservesCurrentGenerationObservations(t *testing.T) {
	config := testSelfHostedConfig()
	config.Generation = 2

	// The inner reconcile already observed BucketReady=True and
	// OIDCObjectsPublished=True at the current generation before failing,
	// and recorded a specific IssuerReady=False/OIDCObjectsPublishFailed
	// via setOIDCObjectsPublishFailed. IAMProviderReady is intentionally
	// not seeded (the inner reconcile never reached the IAM step), so it
	// represents an unobserved sub-state that the outer helper must record.
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionBucketReady, metav1.ConditionTrue, identityv1.ReasonReady, "bucket synced")
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionOIDCObjectsPublished, metav1.ConditionTrue, identityv1.ReasonOIDCObjectsPublished, "objects published")
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIssuerReady, metav1.ConditionFalse, identityv1.ReasonOIDCObjectsPublishFailed, "publish failed inside inner reconcile")

	setSelfHostedIssuerFailureCondition(config, errors.New("provider apply failed"))

	assertConditionAtGeneration(t, config.Status.Conditions, identityv1.ConditionBucketReady, metav1.ConditionTrue, identityv1.ReasonReady, config.Generation)
	assertConditionAtGeneration(t, config.Status.Conditions, identityv1.ConditionOIDCObjectsPublished, metav1.ConditionTrue, identityv1.ReasonOIDCObjectsPublished, config.Generation)
	assertConditionAtGeneration(t, config.Status.Conditions, identityv1.ConditionIssuerReady, metav1.ConditionFalse, identityv1.ReasonOIDCObjectsPublishFailed, config.Generation)
	assertConditionAtGeneration(t, config.Status.Conditions, identityv1.ConditionIAMProviderReady, metav1.ConditionFalse, identityv1.ReasonIssuerReconcileFailed, config.Generation)
}

func assertConditionAtGeneration(t *testing.T, conditions []metav1.Condition, condType string, status metav1.ConditionStatus, reason string, generation int64) {
	t.Helper()

	cond := meta.FindStatusCondition(conditions, condType)
	if cond == nil {
		t.Fatalf("expected condition %s to be present, got nil (have %#v)", condType, conditions)
	}

	if cond.Status != status || cond.Reason != reason || cond.ObservedGeneration != generation {
		t.Fatalf("expected condition %s=%s/%s at generation %d, got %s/%s at generation %d", condType, status, reason, generation, cond.Status, cond.Reason, cond.ObservedGeneration)
	}
}
