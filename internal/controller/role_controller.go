...
func (r *AWSServiceAccountRoleReconciler) setDeliveryConditions(ctx context.Context, log logr.Logger, role *identityv1.AWSServiceAccountRole, delivery identityv1.DeliveryType, inputs *roleReconcileInputs, beforeStatus *identityv1.AWSServiceAccountRoleStatus) (ctrl.Result, error) {
	...
	result, saPatchOp, err := r.reconcileSelfHostedDelivery(ctx, log, role, inputs)
	if err != nil {
		// Surface as a False condition; we log explicitly because returning nil
		// suppresses controller-runtime's own error log.
...
func (r *AWSServiceAccountRoleReconciler) reconcileSelfHostedDelivery(ctx context.Context, log logr.Logger, role *identityv1.AWSServiceAccountRole, inputs *roleReconcileInputs) (ctrl.Result, controllerutil.OperationResult, error) {
	target, err := remoteClusterClient(ctx, r.MCManager, &inputs.resolved)
	if err != nil {
		return ctrl.Result{RequeueAfter: transientRequeue}, controllerutil.OperationResultNone, err
	}
	...
	if err != nil {
		return ctrl.Result{RequeueAfter: transientRequeue}, op, err
	}
...
func (r *AWSServiceAccountRoleReconciler) cleanupRemoteServiceAccountAnnotations(ctx context.Context, log logr.Logger, role *identityv1.AWSServiceAccountRole) error {
	if r.MCManager == nil || role.Status.RoleARN == "" {
		return nil
	}

	config := &identityv1.AWSWorkloadIdentityConfig{}
	if err := r.Get(ctx, client.ObjectKey{Namespace: role.Namespace, Name: identityv1.DefaultName}, config); err != nil {
		log.V(1).Info("skipping remote ServiceAccount annotation cleanup because config is unavailable", "error", err.Error())
	...
	// Inventory may be unavailable during finalizer cleanup (e.g. ClusterProfile
	// already deleted). Best-effort: log and proceed so the finalizer is not
	// blocked indefinitely.
	resolved, err := r.Resolver.Resolve(ctx, role.Namespace)
	if err != nil {
		log.V(1).Info("skipping remote ServiceAccount annotation cleanup", "error", err.Error())

		return nil
	}

	if !resolved.Ready {
		log.V(1).Info("skipping remote ServiceAccount annotation cleanup because inventory is not ready", "awio.condition.reason", resolved.Reason)
	...
	if err != nil {
		log.V(1).Info("skipping remote ServiceAccount annotation cleanup because remote cluster is unavailable", "error", err.Error())

		return nil
	}

	op, err := removeRemoteServiceAccountAnnotations(ctx, target, role.Spec.ServiceAccount, role.Status.RoleARN)
	logRemoteApply(log, metrics.ResourceServiceAccount, serviceAccountIndexKey(role.Spec.ServiceAccount.Namespace, role.Spec.ServiceAccount.Name), op, err)
...
func (r *AWSServiceAccountRoleReconciler) rolesForConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	return requestsForList(ctx, r.Client, &identityv1.AWSServiceAccountRoleList{}, client.InNamespace(obj.GetNamespace()))
}
...
func (r *AWSServiceAccountRoleReconciler) rolesForOperatorConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	return requestsForList(ctx, r.Client, &identityv1.AWSServiceAccountRoleList{})
}
