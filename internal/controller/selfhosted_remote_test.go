...
func TestResolveAnnotationBasedIRSATargetSkipsEKSPodIdentity(t *testing.T) {
	...
	beforeMismatch := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSPodIdentity,
		metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonDeliveryTypeMismatch)
	beforeSelfHostedMismatch := remoteDeliveryCount(t, identityv1.DeliveryTypeSelfHostedIRSA,
		metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonDeliveryTypeMismatch)
	beforeEKSIRSAMismatch := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA,
		metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonDeliveryTypeMismatch)
	...
	afterMismatch := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSPodIdentity,
		metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonDeliveryTypeMismatch)
	if got := afterMismatch - beforeMismatch; got != 1 {
		t.Fatalf("expected EKSPodIdentity delivery_type_mismatch metric delta 1, got %v", got)
	}

	afterSelfHostedMismatch := remoteDeliveryCount(t, identityv1.DeliveryTypeSelfHostedIRSA,
		metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonDeliveryTypeMismatch)
	if got := afterSelfHostedMismatch - beforeSelfHostedMismatch; got != 0 {
		t.Fatalf("expected SelfHostedIRSA delivery_type_mismatch metric delta 0, got %v", got)
	}

	afterEKSIRSAMismatch := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA,
		metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonDeliveryTypeMismatch)
	if got := afterEKSIRSAMismatch - beforeEKSIRSAMismatch; got != 0 {
		t.Fatalf("expected EKSIRSA delivery_type_mismatch metric delta 0, got %v", got)
	}
}
...
func TestResolveAnnotationBasedIRSATargetWaitingInventoryUsesDeliveryType(t *testing.T) {
	...
	beforeEKSIRSA := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA, metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonWaitingInventory)
	beforeSelfHosted := remoteDeliveryCount(t, identityv1.DeliveryTypeSelfHostedIRSA, metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonWaitingInventory)
	...
	afterEKSIRSA := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA, metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonWaitingInventory)

	afterSelfHosted := remoteDeliveryCount(t, identityv1.DeliveryTypeSelfHostedIRSA, metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonWaitingInventory)
...
func TestResolveAnnotationBasedIRSATargetSkipsStaleClusterEvent(t *testing.T) {
	...
	before := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA,
	...
	after := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA,
		metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonStaleClusterEvent)
...
func TestResolveAnnotationBasedIRSATargetClusterUnavailableUsesFixedRetry(t *testing.T) {
	...
	localClient := testConfigClient(t, config, testResolvedClusterProfile(config.Namespace))
	beforeEKSIRSA := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA, metrics.RemoteDeliveryResultError, metrics.RemoteDeliveryReasonClusterUnavail)
	beforeSelfHosted := remoteDeliveryCount(t, identityv1.DeliveryTypeSelfHostedIRSA, metrics.RemoteDeliveryResultError, metrics.RemoteDeliveryReasonClusterUnavail)
	...
	afterEKSIRSA := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA, metrics.RemoteDeliveryResultError, metrics.RemoteDeliveryReasonClusterUnavail)

	afterSelfHosted := remoteDeliveryCount(t, identityv1.DeliveryTypeSelfHostedIRSA, metrics.RemoteDeliveryResultError, metrics.RemoteDeliveryReasonClusterUnavail)
	if got := afterEKSIRSA - beforeEKSIRSA; got != 1 {
...
func remoteDeliveryCount(t *testing.T, deliveryType identityv1.DeliveryType, result, reason string) float64 {
	t.Helper()
	...
	for _, family := range families {
		...
		for _, metric := range family.GetMetric() {
			...
			if labelValue(labels, "delivery_type") != string(deliveryType) ||
				labelValue(labels, "resource") != metrics.ResourceServiceAccount ||
