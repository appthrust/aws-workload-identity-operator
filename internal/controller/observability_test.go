...
func TestRecordForeignChildSkipped(t *testing.T) {
	parent := &identityv1.AWSWorkloadIdentityConfig{
		ObjectMeta: metav1.ObjectMeta{Name: identityv1.DefaultName, Namespace: testInventoryNamespace},
	}
	child := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "signing-key", Namespace: testInventoryNamespace},
	}

	t.Run("emits warning event on parent", func(t *testing.T) {
		recorder := &capturingEventRecorder{}

		recordForeignChildSkipped(recorder, parent, child)
		...
		expected := []recordedEvent{
			{
				...
				reason:    identityv1.ReasonChildOwnershipMismatch,
				action:    eventActionSkipDeleteForeignChild,
				note:      wantNote,
			},
		}
	...
	t.Run("nil recorder is a no-op", func(_ *testing.T) {
		// Mirrors the existing nil-recorder no-op pattern in recordNormalEvent
		// and recordConditionEvents: a nil EventRecorder must not panic.
		recordForeignChildSkipped(nil, parent, child)
	})
}
...
// Messages must reach Eventf as the value argument, not the format string;
// otherwise embedded "%" runes are treated as verbs (e.g. "%!d(MISSING)").
func TestRecordEventsTreatMessageAsLiteral(t *testing.T) {
	obj := &identityv1.AWSWorkloadIdentityConfig{
		ObjectMeta: metav1.ObjectMeta{Name: identityv1.DefaultName, Namespace: testInventoryNamespace},
	}

	t.Run("recordNormalEvent passes message verbatim", func(t *testing.T) {
		recorder := &capturingEventRecorder{}

		const message = "100% repaired (no more %s placeholders)"

		recordNormalEvent(recorder, obj, identityv1.ReasonAnnotationRepaired, eventActionRepairAnnotation, message)

		if len(recorder.events) != 1 {
			t.Fatalf("expected one event, got %#v", recorder.events)
		}

		if recorder.events[0].note != message {
			t.Fatalf("expected note %q (passed verbatim), got %q", message, recorder.events[0].note)
		}
	})

	t.Run("recordConditionEvents passes eventMessage verbatim", func(t *testing.T) {
		recorder := &capturingEventRecorder{}
		// ConditionDeletionBlocked + True routes through warningReason
		// unconditionally, so a synthetic "%"-bearing Reason exercises the
		// format-string risk without depending on the warning allow-list.
		condition := metav1.Condition{
			Type:   identityv1.ConditionDeletionBlocked,
			Status: metav1.ConditionTrue,
			Reason: "Blocked%By%Children",
		}

		recordConditionEvents(recorder, obj, []metav1.Condition{condition})

		if len(recorder.events) != 1 {
			t.Fatalf("expected one event, got %#v", recorder.events)
		}

		want := eventMessage(&condition)
		if recorder.events[0].note != want {
			t.Fatalf("expected note %q (passed verbatim), got %q", want, recorder.events[0].note)
		}
	})
}
