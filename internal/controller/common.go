...
// resultWithDependencySafetyRequeue clamps a reconcile result to the
// dependency steady-state cadence: zero results requeue after the cadence,
// longer requeues are capped, shorter ones pass through. Used by the per-CR
// reconcilers whose Owns/Watches cannot fire when a dependency child does
// not yet exist (e.g. Namespace cache lag, parked-cluster fan-out).
func resultWithDependencySafetyRequeue(result reconcile.Result) reconcile.Result {
	if result.IsZero() {
		return reconcile.Result{RequeueAfter: dependencySteadyStateRequeue}
	}
...
// setACKReadyCondition writes a Ready-style condition that distinguishes
// "ACK has not yet reported ResourceSynced=True" from "ACK is synced but the
// late-initialized identifier (ARN/ID) has not yet been observed on status".
// Callers that have no separate identifier to wait for (e.g. an ACK resource
// whose Synced=True alone implies readiness) pass identifierObserved=true.
func setACKReadyCondition(conditions *[]metav1.Condition, generation int64, condType, kind string, synced, identifierObserved bool) {
	if synced && identifierObserved {
	...
	setCondition(conditions, generation, condType, metav1.ConditionFalse, identityv1.ReasonACKResourceIdentifierPending, "ACK reports "+kind+" as synced but its identifier has not been observed yet")
}
...
func requestsForList(ctx context.Context, log logr.Logger, c client.Reader, controllerName, mapFunc, watchKind string, list client.ObjectList, opts ...client.ListOption) []reconcile.Request {
	log = log.WithValues(
	...
	if err := c.List(ctx, list, opts...); err != nil {
		...
		// would silently drop the watch event. Record the failure on
		// awio_watch_map_list_errors_total so on-call can alert before
		// logging and dropping the event.
		metrics.RecordWatchMapListError(controllerName, mapFunc, watchKind)
