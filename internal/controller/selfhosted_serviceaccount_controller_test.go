...
func TestReconcileRemoteServiceAccountUsesIndexedRoleLookup(t *testing.T) {
	...
	beforeEKSIRSA := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA, metrics.ResourceServiceAccount, metrics.RemoteDeliveryResultSuccess, string(controllerutil.OperationResultUpdated))
	beforeSelfHosted := remoteDeliveryCount(t, identityv1.DeliveryTypeSelfHostedIRSA, metrics.ResourceServiceAccount, metrics.RemoteDeliveryResultSuccess, string(controllerutil.OperationResultUpdated))
	...
	afterEKSIRSA := remoteDeliveryCount(t, identityv1.DeliveryTypeEKSIRSA, metrics.ResourceServiceAccount, metrics.RemoteDeliveryResultSuccess, string(controllerutil.OperationResultUpdated))
	afterSelfHosted := remoteDeliveryCount(t, identityv1.DeliveryTypeSelfHostedIRSA, metrics.ResourceServiceAccount, metrics.RemoteDeliveryResultSuccess, string(controllerutil.OperationResultUpdated))
...
func TestReconcileRemoteServiceAccountSkipsMultipleActiveRoles(t *testing.T) {
	...
	if len(recorder.events) != 1 {
		t.Fatalf("expected 1 warning event, got %d: %#v", len(recorder.events), recorder.events)
	}

	got := recorder.events[0]

	gotSA, ok := got.regarding.(*corev1.ServiceAccount)
	if !ok {
		t.Fatalf("expected event to regard *corev1.ServiceAccount, got %T", got.regarding)
	}

	if gotSA.Name != "app" || gotSA.Namespace != "default" {
		t.Fatalf("expected event regarding remote SA default/app, got %s/%s", gotSA.Namespace, gotSA.Name)
	}

	if got.eventType != corev1.EventTypeWarning ||
		got.reason != identityv1.ReasonInvalidSpec ||
		got.action != eventActionSkipRepairOnRoleConflict {
		t.Fatalf("unexpected event metadata: %#v", got)
	}

	// The note must mention both role keys in lexicographic order so the event
	// is stable across reconciles regardless of indexer iteration order.
	wantOrdered := "[" + testInventoryNamespace + "/role-a " + testInventoryNamespace + "/role-b]"
	if !strings.Contains(got.note, wantOrdered) {
		t.Fatalf("expected event note to contain lex-sorted role keys %q, got %q", wantOrdered, got.note)
	}
}
