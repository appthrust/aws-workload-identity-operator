...
// TestRolesForClusterProfileDirectEnqueuesNamespace verifies that a direct
// ClusterProfile (Namespace == Name) maps to the workload namespace named by
// the profile's own Name.
func TestRolesForClusterProfileDirectEnqueuesNamespace(t *testing.T) {
	role := &identityv1.AWSServiceAccountRole{
		ObjectMeta: metav1.ObjectMeta{Namespace: "ws-b", Name: "role-direct"},
	}
	roleOther := &identityv1.AWSServiceAccountRole{
		ObjectMeta: metav1.ObjectMeta{Namespace: "ws-a", Name: "role-elsewhere"},
	}

	c := fake.NewClientBuilder().
		WithScheme(testControllerScheme(t)).
		WithObjects(role, roleOther).
		Build()

	r := &AWSServiceAccountRoleReconciler{Client: c}

	profile := &clusterinventoryv1alpha1.ClusterProfile{
		ObjectMeta: metav1.ObjectMeta{Namespace: "ws-b", Name: "ws-b"},
	}

	requests := r.rolesForClusterProfile(context.Background(), profile)
	got := sortedRequestKeys(requests)
	want := []string{"ws-b/role-direct"}
