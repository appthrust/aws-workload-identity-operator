...
// testInlinePolicyDocument is the canonical valid JSON policy used to drive
// reconcileGeneratedPolicy through its upsert path in tests.
const testInlinePolicyDocument = `{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Action":["s3:GetObject"],"Resource":"*"}]}`
...
// reconcileGeneratedPolicy: a malformed JSON payload must publish
...
func TestRoleReconcileNormalInvalidPolicyDocumentSetsPolicyReadyFalse(t *testing.T) {
	role := testAWSServiceAccountRole()
	// Malformed JSON fails identityaws.PolicyDocumentString at the json.Unmarshal
	// probe inside reconcileGeneratedPolicy.
	role.Spec.PolicyDocument = "not valid json"
