...
// TestRoleReconcileNormalInvalidPolicyDocumentSetsPolicyReadyFalse is the
// regression test for the PolicyDocumentString canonicalize-error branch of
// reconcileGeneratedPolicy: a malformed Statement.Raw entry must publish
...
// reconcileGeneratedPolicy directly because a malformed Statement.Raw also
// makes the fake client's serializer reject any Patch/List touching the role,
// which would mask the in-memory condition assertions we actually care about.
func TestRoleReconcileNormalInvalidPolicyDocumentSetsPolicyReadyFalse(t *testing.T) {
	role := testAWSServiceAccountRole()
	// Malformed JSON in Statement.Raw fails identityaws.PolicyDocumentString
	// at the json.Marshal step inside reconcileGeneratedPolicy.
	role.Spec.PolicyDocument = &identityv1.AWSIAMPolicyDocument{
		Version: "2012-10-17",
		Statement: []identityv1.AWSIAMPolicyStatement{
