...
// resolveRemoteCluster fetches the workload cluster handle for resolved.
// multicluster.ErrClusterNotFound is preserved for `errors.Is`.
func resolveRemoteCluster(ctx context.Context, getter remoteClusterGetter, resolved *inventory.Resolution) (cluster.Cluster, error) {
	if getter == nil {
...
// remoteClusterClient returns the cached workload-cluster client.
func remoteClusterClient(ctx context.Context, getter remoteClusterGetter, resolved *inventory.Resolution) (client.Client, error) {
	target, err := resolveRemoteCluster(ctx, getter, resolved)
	if err != nil {
		return nil, err
	}
...
// remoteClusterTargets returns both the cached client and uncached APIReader in
// a single GetCluster call. The uncached reader bypasses selector-scoped caches
// (used by ownership-verification Gets); the cached client backs apply paths.
func remoteClusterTargets(ctx context.Context, getter remoteClusterGetter, resolved *inventory.Resolution) (client.Client, client.Reader, error) {
	target, err := resolveRemoteCluster(ctx, getter, resolved)
	if err != nil {
		return nil, nil, err
	}

	return target.GetClient(), target.GetAPIReader(), nil
}
...
func loadConfigMatching(
	...
	if !accept(config.Spec.Type) {
		metrics.RecordRemoteDelivery(string(config.Spec.Type), resource, metrics.RemoteDeliveryResultSkipped, metrics.RemoteDeliveryReasonDeliveryTypeMismatch)
