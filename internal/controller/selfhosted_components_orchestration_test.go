...
func TestReconcileNormalSelfHostedRuntimeFailureUsesFixedRetryAndDoesNotDiscardIssuerStatus(t *testing.T) {
	ctx := logr.NewContext(context.Background(), logr.Discard())
	config := testSelfHostedConfig()
	publisher := &fakeOIDCIssuerPublisher{}
	...
	result, err := reconciler.reconcileNormal(ctx, config)
	if err != nil {
		t.Fatalf("expected runtime error to be handled as a fixed retry, got result=%#v err=%v", result, err)
	}
...
func TestReconcileNormalSelfHostedReportsIssuerErrorWithoutIgnoredRuntimeRetry(t *testing.T) {
	ctx := logr.NewContext(context.Background(), logr.Discard())
	config := testSelfHostedConfig()
	...
	result, err := reconciler.reconcileNormal(ctx, config)
	if err == nil {
		t.Fatalf("expected issuer error, got result=%#v", result)
	}

	if !strings.Contains(err.Error(), issuerErr.Error()) || strings.Contains(err.Error(), runtimeErr.Error()) {
		t.Fatalf("expected only issuer failure to bubble while runtime uses fixed retry, got %v", err)
	}
