...
func TestReconcileNormalSelfHostedIssuerFailureDoesNotSkipRuntimeStatusOrReturnIgnoredRuntimeRetry(t *testing.T) {
	...
	publisher := &fakeOIDCIssuerPublisher{ensureErr: errors.New("s3 access denied")}
	remoteClient := fakeClient(t, bootstrappedWebhookNamespace(), availableWebhookDeployment())
...
func TestReconcileNormalSelfHostedSuccessMergesIssuerAndRuntimeStatus(t *testing.T) {
	...
	if stored.Status.SelfHostedIssuer == nil || stored.Status.SelfHostedIssuer.BucketName != testIssuerBucketName(config) {
		t.Fatalf("expected issuer bucket status to merge, got %#v", stored.Status.SelfHostedIssuer)
	}

	if stored.Status.SelfHostedIssuer.Publication == nil || stored.Status.SelfHostedIssuer.Publication.SigningKeyID == "" {
		t.Fatalf("expected issuer publication status to merge, got %#v", stored.Status.SelfHostedIssuer)
	}
...
func TestReconcileNormalSelfHostedRuntimeFailurePersistsStatusAndBubblesError(t *testing.T) {
	...
	if stored.Status.SelfHostedIssuer.Publication == nil || stored.Status.SelfHostedIssuer.Publication.SigningKeyID == "" {
		t.Fatalf("expected issuer publication status to merge despite runtime failure, got %#v", stored.Status.SelfHostedIssuer)
	}
...
func TestReconcileNormalSelfHostedReportsBothIssuerAndRuntimeErrors(t *testing.T) {
	...
	publisher := &fakeOIDCIssuerPublisher{ensureErr: issuerErr}
	localClient := testSelfHostedComponentsClient(t,
		config,
		testOperatorConfig(),
		testResolvedClusterProfile(config.Namespace),
...
func TestReconcileSelfHostedWebhookRuntimeBubblesApplyError(t *testing.T) {
	...
	operatorConfig := testOperatorConfig()
	resolved := readyResolution()
	updateErr := errors.New("simulated namespace update failure")
	// Seed the namespace with the minimum labels required by the adoption probe
	// (managed-by + runtime) but omit the rest of webhookRuntimeLabels() so the
	// reconcile-side label merge diff is non-empty and createOrUpdate issues
	// an Update on the bootstrapped namespace — exercising the named
	// namespaces:update verb, where the injected interceptor fires.
	seededNamespace := &corev1.Namespace{
	...
	if !errors.Is(err, updateErr) {
		t.Fatalf("expected wrapped error to satisfy errors.Is on the injected update error, got %v", err)
	}
...
func TestSelfHostedStatusMergesOwnedConditionsBeforeReadyComputed(t *testing.T) {
	...
	issuerConfig.Status.SelfHostedIssuer = &identityv1.AWSWorkloadIdentityConfigSelfHostedIssuerStatus{
		BucketName: testIssuerBucketNameValue,
		Publication: &identityv1.AWSWorkloadIdentityConfigSelfHostedIssuerPublicationStatus{
			BucketName:      testIssuerBucketNameValue,
			IssuerURL:       "https://" + testIssuerBucketNameValue + ".s3.ap-northeast-1.amazonaws.com",
			SigningKeyID:    "key-id",
			ObjectSetDigest: "sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc",
		},
	}
...
func testResolvedClusterProfile(namespace string) *clusterinventoryv1alpha1.ClusterProfile {
	return &clusterinventoryv1alpha1.ClusterProfile{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: namespace,
			Labels: map[string]string{
				inventory.LabelOCMClusterName:                   namespace,
