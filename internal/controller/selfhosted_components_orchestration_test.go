...
func TestReconcileNormalSelfHostedIssuerFailureDoesNotSkipRuntimeStatus(t *testing.T) {
	...
	if result.RequeueAfter <= transientRequeue {
		t.Fatalf("expected runtime certificate requeue result to be preserved, got %#v", result)
	}
...
func TestReconcileNormalSelfHostedSuccessMergesIssuerAndRuntimeStatus(t *testing.T) {
	...
	localClient := testSelfHostedComponentsClient(t,
		...
		testResolvedClusterProfile(config.Namespace),
		testBucket(config, true),
		testOIDCProvider(config, true, "arn:aws:iam::123456789012:oidc-provider/example"),
	)
	reconciler := testSelfHostedComponentsReconciler(t, localClient, publisher, remoteClient, nil)
...
func TestReconcileNormalSelfHostedRuntimeNotReadyUsesReadinessRetry(t *testing.T) {
	...
	stored := getStoredConfig(t, localClient, config)
	assertCondition(t, stored.Status.Conditions, identityv1.ConditionWebhookRuntimeReady, metav1.ConditionFalse, identityv1.ReasonWaitingForWebhookDeployment)
...
func TestReconcileNormalSelfHostedRuntimeFailureDoesNotDiscardIssuerStatus(t *testing.T) {
	ctx := logr.NewContext(context.Background(), logr.Discard())
	config := testSelfHostedConfig()
	publisher := &fakeOIDCIssuerPublisher{}
	...
	result, err := reconciler.reconcileNormal(ctx, config)
	if err == nil || !strings.Contains(err.Error(), remoteErr.Error()) {
		t.Fatalf("expected runtime error, got result=%#v err=%v", result, err)
	}

	if result.RequeueAfter != transientRequeue {
		t.Fatalf("expected runtime failure result to be preserved, got %#v", result)
	}
...
func TestReconcileNormalSelfHostedJoinsIssuerAndRuntimeErrors(t *testing.T) {
	ctx := logr.NewContext(context.Background(), logr.Discard())
	config := testSelfHostedConfig()
	...
	result, err := reconciler.reconcileNormal(ctx, config)
	if err == nil {
		t.Fatalf("expected joined issuer and runtime errors, got result=%#v", result)
	}

	if !strings.Contains(err.Error(), issuerErr.Error()) || !strings.Contains(err.Error(), runtimeErr.Error()) {
		t.Fatalf("expected joined error to contain issuer and runtime failures, got %v", err)
	}
...
func testOIDCProvider(config *identityv1.AWSWorkloadIdentityConfig, synced bool, arn string) *iamv1alpha1.OpenIDConnectProvider {
