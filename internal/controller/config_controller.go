...
//
// +kubebuilder:rbac:groups=aws.identity.appthrust.io,resources=awsworkloadidentityconfigs,verbs=get;list;watch;patch
...
// Secrets stays cluster-wide: the built-in RBAC authorizer cannot label-filter list/watch and
// signing-key Secrets live in user-provided AWSWorkloadIdentityConfig namespaces.
// LocalSecretCacheByObject in local_cache.go strips Data on foreign Secrets as the
// compensating control; narrowing requires a breaking API change.
...
type AWSWorkloadIdentityConfigReconciler struct {
	...
	SigningSecretReader              client.Reader
...
type SelfHostedIssuerPublisher interface {
	...
	EnsureOIDCIssuer(ctx context.Context, bucket string, publication oidc.IssuerPublication) (bool, error)
}
...
func (r *AWSWorkloadIdentityConfigReconciler) reconcileNormal(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) (ctrl.Result, error) {
	...
	result, componentErr := r.reconcileConfigComponents(ctx, log, config, operatorConfig, &resolved)
	...
	return resultWithDependencySafetyRequeue(result), nil
}

func (r *AWSWorkloadIdentityConfigReconciler) reconcileConfigComponents(
	ctx context.Context,
	log logr.Logger,
	config *identityv1.AWSWorkloadIdentityConfig,
	...
	case identityv1.DeliveryTypeSelfHostedIRSA:
		return r.reconcileSelfHostedComponents(ctx, log, config, operatorConfig, resolved)
	case identityv1.DeliveryTypeEKSIRSA:
		return ctrl.Result{}, r.reconcileEKSIRSAConfig(ctx, log, config)
	case identityv1.DeliveryTypeEKSPodIdentity:
		setEKSPodIdentityConfigNotRequiredStatus(config)
	default:
		setEKSPodIdentityConfigNotRequiredStatus(config)
	}
...
func setEKSPodIdentityConfigNotRequiredStatus(config *identityv1.AWSWorkloadIdentityConfig) {
	config.Status.ACKResources = nil
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIssuerReady, metav1.ConditionTrue, identityv1.ReasonNotRequired, "EKSPodIdentity does not use an issuer")
...
func mergeSelfHostedIssuerStatus(dst, src *identityv1.AWSWorkloadIdentityConfig) {
	...
	dst.Status.SelfHostedIssuer = src.Status.SelfHostedIssuer
...
func setResolvedClusterStatus(config *identityv1.AWSWorkloadIdentityConfig, resolved *inventory.Resolution) {
	if config.Spec.Type.UsesAnnotationBasedIRSA() && resolved.Ready {
...
func resetSelfHostedStatus(config *identityv1.AWSWorkloadIdentityConfig) {
	resetIssuerStatus(config)
	config.Status.ResolvedClusterName = ""
	resetSelfHostedWebhookRuntimeStatus(config)
}

func resetIssuerStatus(config *identityv1.AWSWorkloadIdentityConfig) {
	config.Status.SelfHostedIssuer = nil
	config.Status.IssuerHostPath = ""
	config.Status.OIDCProviderARN = ""
}

func resetSelfHostedWebhookRuntimeStatus(config *identityv1.AWSWorkloadIdentityConfig) {
	config.Status.WebhookRuntimeNamespace = ""
	config.Status.WebhookRuntimeCertNotAfter = nil
}

func (r *AWSWorkloadIdentityConfigReconciler) reconcileEKSIRSAConfig(ctx context.Context, log logr.Logger, config *identityv1.AWSWorkloadIdentityConfig) error {
	config.Status.ACKResources = nil
	resetIssuerStatus(config)
	resetSelfHostedWebhookRuntimeStatus(config)
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionWebhookRuntimeReady, metav1.ConditionTrue, identityv1.ReasonNotRequired, "EKSIRSA does not use the self-hosted webhook runtime")

	if config.Spec.EKSIRSA == nil {
		setEKSIRSAInvalidSpec(config, "spec.eksIRSA is required when spec.type is EKSIRSA")

		return nil
	}

	issuerHostPath, err := identityaws.NormalizeIssuerURL(config.Spec.EKSIRSA.IssuerURL)
	if err != nil {
		config.Status.IssuerHostPath = ""
		config.Status.OIDCProviderARN = ""
		setEKSIRSAInvalidSpec(config, err.Error())

		return nil
	}

	config.Status.IssuerHostPath = issuerHostPath

	switch config.Spec.EKSIRSA.OIDCProvider.Management {
	case identityv1.OIDCProviderManagementManaged:
		providerSynced, providerStatus, err := r.reconcileOIDCProvider(ctx, log, config)
		if err != nil {
			setEKSIRSAIssuerFailure(config, err)

			return err
		}

		config.Status.ACKResources = []identityv1.ACKResourceStatus{providerStatus}
		if providerSynced && config.Status.OIDCProviderARN != "" {
			setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIssuerReady, metav1.ConditionTrue, identityv1.ReasonReady, "EKS IRSA issuer resources are synced")
		} else {
			setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIssuerReady, metav1.ConditionFalse, identityv1.ReasonWaitingForACK, "waiting for IAM OpenIDConnectProvider ARN")
		}
	case identityv1.OIDCProviderManagementExternal:
		providerARN := config.Spec.EKSIRSA.OIDCProvider.ARN
		if err := identityaws.ValidateOIDCProviderARNMatchesIssuer(providerARN, issuerHostPath); err != nil {
			config.Status.OIDCProviderARN = ""
			setEKSIRSAInvalidSpec(config, err.Error())
		...
		config.Status.OIDCProviderARN = providerARN
		setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIAMProviderReady, metav1.ConditionTrue, identityv1.ReasonReady, "external IAM OpenIDConnectProvider ARN matches issuer")
		setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIssuerReady, metav1.ConditionTrue, identityv1.ReasonReady, "EKS IRSA external issuer is configured")
	default:
		config.Status.OIDCProviderARN = ""
		setEKSIRSAInvalidSpec(config, fmt.Sprintf("unsupported EKSIRSA OIDC provider management %q", config.Spec.EKSIRSA.OIDCProvider.Management))
	}

	return nil
}

func setEKSIRSAInvalidSpec(config *identityv1.AWSWorkloadIdentityConfig, message string) {
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIAMProviderReady, metav1.ConditionFalse, identityv1.ReasonInvalidSpec, message)
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIssuerReady, metav1.ConditionFalse, identityv1.ReasonInvalidSpec, message)
}

func setEKSIRSAIssuerFailure(config *identityv1.AWSWorkloadIdentityConfig, err error) {
	config.Status.OIDCProviderARN = ""
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIAMProviderReady, metav1.ConditionFalse, identityv1.ReasonIssuerReconcileFailed, err.Error())
...
func (r *AWSWorkloadIdentityConfigReconciler) reconcileSelfHostedWebhookRuntime(ctx context.Context, log logr.Logger, config *identityv1.AWSWorkloadIdentityConfig, operatorConfig *identityv1.AWSWorkloadIdentityOperatorConfig, resolved *inventory.Resolution) (ctrl.Result, error) {
	...
	target, targetReader, result, err := r.resolveSelfHostedWebhookTarget(ctx, log, config, resolved)
	...
	if err != nil {
		reason := identityv1.ReasonWebhookRuntimeApplyFailed
		if isErrUnmanagedRemoteRuntime(err) {
			reason = identityv1.ReasonRemoteRuntimeUnmanaged
		}

		setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionWebhookRuntimeReady, metav1.ConditionFalse, reason, err.Error())

		log.Error(err, "self-hosted webhook runtime deferred", logKeyConditionReason, reason)
...
// resolveSelfHostedWebhookTarget resolves both the cached remote client and
// the uncached remote APIReader for the workload cluster backing config. The
// uncached reader is required by applyRemoteWebhookRuntime to detect
// pre-existing unlabeled objects that the selector-scoped remote cache hides
// from the cached client; both must resolve cleanly before any apply work
// begins. A returned non-nil err (or nil target) means the caller must short-
// circuit with result.
func (r *AWSWorkloadIdentityConfigReconciler) resolveSelfHostedWebhookTarget(ctx context.Context, log logr.Logger, config *identityv1.AWSWorkloadIdentityConfig, resolved *inventory.Resolution) (client.Client, client.Reader, ctrl.Result, error) {
	handleErr := func(err error, what string) (ctrl.Result, error) {
		...
		return ctrl.Result{}, fmt.Errorf("resolve remote %s for self-hosted webhook runtime: %w", what, err)
	}

	target, targetReader, err := remoteClusterTargets(ctx, r.MCManager, resolved)
	if err != nil {
		result, retErr := handleErr(err, "cluster")

		return nil, nil, result, retErr
	}

	return target, targetReader, ctrl.Result{}, nil
}
...
// setSelfHostedIssuerFailureCondition reconciles the issuer-owned conditions
// after reconcileSelfHostedIssuer returned an error. Early error-return paths
// inside reconcileSelfHostedIssuer can leave one or more of the four conditions
// (BucketReady, OIDCObjectsPublished, IAMProviderReady, IssuerReady) at a
// prior-generation observation; persisting that as the current generation's
// status would lie about which sub-state the controller has actually verified
// this reconcile. For each condition, preserve any observation already made on
// the current generation (it carries the more specific upstream reason); for
// the rest, record the issuer failure so the persisted status reflects current
// observation rather than stale prior-True.
func setSelfHostedIssuerFailureCondition(config *identityv1.AWSWorkloadIdentityConfig, err error) {
	msg := identityaws.SanitizeError(err)
	...
	for _, condType := range []string{
		...
		cond := meta.FindStatusCondition(config.Status.Conditions, condType)
		if cond != nil && cond.ObservedGeneration == config.Generation {
			continue
		}

		setCondition(&config.Status.Conditions, config.Generation, condType, metav1.ConditionFalse, identityv1.ReasonIssuerReconcileFailed, msg)
	}
}
...
func (r *AWSWorkloadIdentityConfigReconciler) reconcileSelfHostedIssuer(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) error {
	...
	setSelfHostedIssuerBucketStatus(config, bucketName)
	...
	publication, err := oidc.RenderIssuerPublication(config.Status.IssuerURL(), publicKeyPEM, keyID)
	if err != nil {
		err = fmt.Errorf("render self-hosted OIDC issuer publication: %w", err)
		setOIDCObjectsPublishFailed(config, err)
	...
	if !bucketSynced {
		// Bucket not synced: ACK may be (re-)creating it, so previously verified
		// objects are no longer guaranteed to exist.
		clearSelfHostedIssuerPublicationStatus(config)
	...
	if err := r.ensureOIDCObjectsPublished(ctx, config, publication); err != nil {
		return err
	}
...
func (r *AWSWorkloadIdentityConfigReconciler) reconcileSelfHostedBucket(ctx context.Context, log logr.Logger, config *identityv1.AWSWorkloadIdentityConfig) (bool, identityv1.ACKResourceStatus, error) {
	...
	setACKReadyCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionBucketReady, "S3 Bucket", synced, true)

	return synced, status, nil
}

func setSelfHostedIssuerBucketStatus(config *identityv1.AWSWorkloadIdentityConfig, bucketName string) {
	...
	config.Status.SelfHostedIssuer.BucketName = bucketName
}

func clearSelfHostedIssuerPublicationStatus(config *identityv1.AWSWorkloadIdentityConfig) {
	if config.Status.SelfHostedIssuer == nil {
		return
	}

	config.Status.SelfHostedIssuer.Publication = nil
}

func setSelfHostedIssuerPublicationStatus(config *identityv1.AWSWorkloadIdentityConfig, bucketName string, publication oidc.IssuerPublication) {
	setSelfHostedIssuerBucketStatus(config, bucketName)
	config.Status.SelfHostedIssuer.Publication = &identityv1.AWSWorkloadIdentityConfigSelfHostedIssuerPublicationStatus{
		BucketName:      bucketName,
		IssuerURL:       publication.IssuerURL,
		SigningKeyID:    publication.SigningKeyID,
		ObjectSetDigest: publication.ObjectSetDigest,
	}
}

func selfHostedIssuerBucketName(config *identityv1.AWSWorkloadIdentityConfig) string {
	if config.Status.SelfHostedIssuer != nil && config.Status.SelfHostedIssuer.BucketName != "" {
		return config.Status.SelfHostedIssuer.BucketName
	}
...
func (r *AWSWorkloadIdentityConfigReconciler) reconcileOIDCProvider(ctx context.Context, log logr.Logger, config *identityv1.AWSWorkloadIdentityConfig) (bool, identityv1.ACKResourceStatus, error) {
	...
	op, err := createOrUpdate(ctx, r.Client, r.Scheme, config, current, func() error {
		spec := desired.Spec
		preserveOIDCProviderLateInitializedFields(&spec, &current.Spec)
	...
	ackSynced := identityaws.IsACKSynced(current.Status.Conditions)
	identifierObserved := config.Status.OIDCProviderARN != ""
	synced := ackSynced && identifierObserved
	setACKReadyCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIAMProviderReady, "IAM OpenIDConnectProvider", ackSynced, identifierObserved)

	return synced, status, nil
}

func preserveOIDCProviderLateInitializedFields(desired, current *iamv1alpha1.OpenIDConnectProviderSpec) {
	if desired.Thumbprints == nil && current != nil && current.Thumbprints != nil {
		desired.Thumbprints = current.Thumbprints
	}
}
...
// ensureOIDCObjectsPublished verifies S3 metadata for every desired issuer
// object and rewrites any object whose operator-owned metadata is missing or
// stale. Status is updated only after S3 has been confirmed current.
func (r *AWSWorkloadIdentityConfigReconciler) ensureOIDCObjectsPublished(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig, publication oidc.IssuerPublication) error {
	publisher, err := r.selfHostedPublisher(ctx, config.Spec.Region)
	if err != nil {
		return err
	}

	bucketName := selfHostedIssuerBucketName(config)

	changed, err := publisher.EnsureOIDCIssuer(ctx, bucketName, publication)
	if err != nil {
		err = fmt.Errorf("publish self-hosted OIDC issuer objects: %w", err)
		setOIDCObjectsPublishFailed(config, err)
	...
	if changed {
		logf.FromContext(ctx).V(1).Info("self-hosted OIDC issuer objects updated", "bucket", bucketName, "objectSetDigest", publication.ObjectSetDigest)
	}

	setSelfHostedIssuerPublicationStatus(config, bucketName, publication)

	return nil
}

func setOIDCObjectsPublishFailed(config *identityv1.AWSWorkloadIdentityConfig, err error) {
	msg := identityaws.SanitizeError(err)
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionOIDCObjectsPublished, metav1.ConditionFalse, identityv1.ReasonOIDCObjectsPublishFailed, msg)
	setCondition(&config.Status.Conditions, config.Generation, identityv1.ConditionIssuerReady, metav1.ConditionFalse, identityv1.ReasonOIDCObjectsPublishFailed, msg)
}

// deleteSelfHostedOIDCObjects deletes the OIDC discovery and JWKS objects from
// the self-hosted S3 bucket, but only after confirming the bucket ACK CR is
// still controlled by this AWSWorkloadIdentityConfig. The bucket name is
// deterministic (identityaws.BucketName(namespace, region)), so without this
// ownership guard a foreign config that has reclaimed the same bucket name
// would have its live OIDC discovery / JWKS objects destroyed (ABA
// collateral-deletion). The Bucket ACK CR is the authoritative ownership
// witness; this mirrors the controllerRef/UID guard in deleteConfigChildIfOwned.
func (r *AWSWorkloadIdentityConfigReconciler) deleteSelfHostedOIDCObjects(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig, bucketName string) error {
	...
	key := client.ObjectKey{Namespace: config.Namespace, Name: bucketName}
	...
	if err := r.Get(ctx, key, bucket); err != nil {
		...
		return fmt.Errorf("get %T %s/%s: %w", bucket, key.Namespace, key.Name, err)
	}

	if !isConfigChildOwnedBy(config, bucket) {
		log.Info("skipping delete: S3 issuer objects target bucket not controlled by this AWSWorkloadIdentityConfig",
			"awio.child.kind", fmt.Sprintf("%T", bucket),
			"awio.child.namespace", bucket.Namespace,
			"awio.child.name", bucket.Name)

		recordForeignChildSkipped(r.Recorder, config, bucket)
...
func (r *AWSWorkloadIdentityConfigReconciler) reconcileSigningSecret(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) ([]byte, string, error) {
	log := logf.FromContext(ctx)
	key := client.ObjectKey{Name: identityaws.SigningKeySecretName(config), Namespace: config.Namespace}
	...
	op, err := createOrUpdate(ctx, r.Client, r.Scheme, config, secret, func() error {
		if err := r.restoreSigningSecretData(ctx, key, secret); err != nil {
			return err
		}
...
func (r *AWSWorkloadIdentityConfigReconciler) restoreSigningSecretData(ctx context.Context, key client.ObjectKey, secret *corev1.Secret) error {
	if hasSigningSecretData(secret.Data) || r.SigningSecretReader == nil {
		return nil
	}

	live := &corev1.Secret{}
	if err := r.SigningSecretReader.Get(ctx, key, live); err != nil {
		if apierrors.IsNotFound(err) {
			return nil
		}

		return fmt.Errorf("get signing Secret %s/%s with uncached reader: %w", key.Namespace, key.Name, err)
	}

	if !hasSigningSecretData(live.Data) {
		return nil
	}
	...
	for _, name := range []string{identityaws.SigningKeyPrivateKey, identityaws.SigningKeyPublicKey} {
		if len(secret.Data[name]) == 0 && len(live.Data[name]) > 0 {
			secret.Data[name] = append([]byte(nil), live.Data[name]...)
		}
	}

	if secret.Type == "" {
		secret.Type = live.Type
	}

	return nil
}

func hasSigningSecretData(data map[string][]byte) bool {
	return len(data[identityaws.SigningKeyPrivateKey]) > 0 && len(data[identityaws.SigningKeyPublicKey]) > 0
}
...
// public key into the Secret's annotations, overwriting any annotation that
// no longer matches the derivation so the persisted ID stays in lockstep with
...
func (r *AWSWorkloadIdentityConfigReconciler) reconcileDelete(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) (ctrl.Result, error) {
	...
	if err := r.deleteRemoteWebhookRuntime(ctx, config); err != nil {
		reason := identityv1.ReasonRemoteClusterUnavailable
		if isErrUnmanagedRemoteRuntime(err) {
			reason = identityv1.ReasonRemoteRuntimeUnmanaged
		}

		if patchErr := r.markDeletionBlocked(ctx, log, config, reason, err.Error()); patchErr != nil {
			return ctrl.Result{}, patchErr
		}
	...
	if err := r.deleteConfigChildren(ctx, config); err != nil {
		return ctrl.Result{}, fmt.Errorf("delete config children: %w", err)
	}
	...
	if err := removeFinalizer(ctx, r.Client, r.Recorder, log, config, identityv1.ConfigFinalizer); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *AWSWorkloadIdentityConfigReconciler) deleteConfigChildren(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) error {
	switch config.Spec.Type {
	case identityv1.DeliveryTypeSelfHostedIRSA:
		return r.deleteSelfHostedChildren(ctx, config)
	case identityv1.DeliveryTypeEKSIRSA:
		return r.deleteEKSIRSAChildren(ctx, config)
	case identityv1.DeliveryTypeEKSPodIdentity:
		return nil
	default:
		return nil
...
func (r *AWSWorkloadIdentityConfigReconciler) deleteRemoteWebhookRuntimeStrict(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) error {
	...
	target, targetReader, err := remoteClusterTargets(ctx, r.MCManager, &resolved)
	if err != nil {
		return fmt.Errorf("resolve remote cluster for webhook runtime cleanup: %w", err)
	}
	...
	return deleteRemoteWebhookRuntime(ctx, targetReader, target, namespace)
}
...
func (r *AWSWorkloadIdentityConfigReconciler) deleteSelfHostedChildren(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) error {
	...
	g.Go(func() error {
		...
		return r.deleteConfigChildIfOwned(gCtx, log, config, obj)
	})
	...
	g.Go(func() error {
		...
		return r.deleteConfigChildIfOwned(gCtx, log, config, bucket)
	})
...
// deleteConfigChildIfOwned deletes a generated child resource only after
// confirming its controllerRef UID matches config. Without this guard,
// name-based deletion of deterministically-named children would risk ABA
// collateral-deletion of a foreign resource sharing the same name.
func (r *AWSWorkloadIdentityConfigReconciler) deleteConfigChildIfOwned(ctx context.Context, log logr.Logger, config *identityv1.AWSWorkloadIdentityConfig, child client.Object) error {
	...
	if err := r.Get(ctx, key, child); err != nil {
		...
		return fmt.Errorf("get %T %s/%s: %w", child, key.Namespace, key.Name, err)
	}

	if !isConfigChildOwnedBy(config, child) {
		log.Info("skipping delete: child is not controlled by this AWSWorkloadIdentityConfig",
			"awio.child.kind", fmt.Sprintf("%T", child),
			"awio.child.namespace", key.Namespace,
			"awio.child.name", key.Name)

		recordForeignChildSkipped(r.Recorder, config, child)

		return nil
	}

	if err := client.IgnoreNotFound(r.Delete(ctx, child)); err != nil {
		return fmt.Errorf("delete %T %s/%s: %w", child, key.Namespace, key.Name, err)
	}

	return nil
}

func (r *AWSWorkloadIdentityConfigReconciler) deleteEKSIRSAChildren(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) error {
	...
	return r.deleteConfigChildIfOwned(ctx, logf.FromContext(ctx), config, obj)
}

func isConfigChildOwnedBy(config *identityv1.AWSWorkloadIdentityConfig, obj client.Object) bool {
	if config == nil || obj == nil || config.UID == "" {
		return false
	}

	if owner := metav1.GetControllerOf(obj); owner != nil && owner.UID == config.UID {
		return true
	}

	return obj.GetLabels()[identityv1.LabelConfigUID] == string(config.UID)
}
...
func (r *AWSWorkloadIdentityConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	builder := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&identityv1.AWSServiceAccountRole{},
			handler.EnqueueRequestsFromMapFunc(configForDeletedRole),
			crbuilder.WithPredicates(roleDeletedForConfigPredicate(metrics.ControllerConfig))).
		WithOptions(controller.Options{MaxConcurrentReconciles: r.MaxConcurrentReconciles})

	bucketGVK := s3v1alpha1.GroupVersion.WithKind(ackChildKindBucket)

	bucketPresent, err := hasMapping(mgr.GetRESTMapper(), bucketGVK)
	if err != nil {
		return fmt.Errorf("probe optional CRD %s: %w", bucketGVK, err)
	}

	if bucketPresent {
		builder = builder.Owns(&s3v1alpha1.Bucket{})
	} else {
		builder = builder.Watches(
			&apiextensionsv1.CustomResourceDefinition{},
			handler.EnqueueRequestsFromMapFunc(r.configsForAckChildCRD),
			crbuilder.WithPredicates(ackChildCRDChangedPredicate(metrics.ControllerConfig, ackChildCRDNameBucket)),
		)
	}
...
func (r *AWSWorkloadIdentityConfigReconciler) configsForOperatorConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	return requestsForList(ctx, log, r.Client, metrics.ControllerConfig, "configsForOperatorConfig", "AWSWorkloadIdentityOperatorConfig", &identityv1.AWSWorkloadIdentityConfigList{})
}
...
// configForDeletedRole enqueues the singleton AWSWorkloadIdentityConfig in the
// role's namespace so reconcileDelete wakes promptly when the last child role
// is removed, instead of waiting for the deletion safety net's 30s requeue.
func configForDeletedRole(ctx context.Context, obj client.Object) []reconcile.Request {
	if _, ok := obj.(*identityv1.AWSServiceAccountRole); !ok {
		return nil
	}

	log := watchMapLogger(ctx, metrics.ControllerConfig, "configForDeletedRole", "AWSServiceAccountRole", obj)
	log.V(1).Info("enqueueing AWSWorkloadIdentityConfig for deleted AWSServiceAccountRole")
...
func (r *AWSWorkloadIdentityConfigReconciler) configsForAckChildCRD(ctx context.Context, obj client.Object) []reconcile.Request {
	log := watchMapLogger(ctx, metrics.ControllerConfig, "configsForAckChildCRD", "CustomResourceDefinition", obj)

	return requestsForList(ctx, log, r.Client, metrics.ControllerConfig, "configsForAckChildCRD", "CustomResourceDefinition", &identityv1.AWSWorkloadIdentityConfigList{})
}
