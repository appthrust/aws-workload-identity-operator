...
func mergeSelfHostedIssuerStatus(dst, src *identityv1.AWSWorkloadIdentityConfig) {
	...
	dst.Status.PublishedIssuerHash = src.Status.PublishedIssuerHash
...
func resetIssuerStatus(config *identityv1.AWSWorkloadIdentityConfig) {
	...
	config.Status.PublishedIssuerHash = ""
}
...
func (r *AWSWorkloadIdentityConfigReconciler) reconcileSelfHostedIssuer(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig) error {
	...
	if !bucketSynced {
		...
		// next sync by clearing the recorded fingerprint.
		config.Status.PublishedIssuerHash = ""
		config.Status.PublishedKeyID = ""
	...
	if err := r.ensureOIDCObjectsPublished(ctx, config, publicKeyPEM, keyID); err != nil {
		return err
	}
...
// ensureOIDCObjectsPublished re-publishes the self-hosted OIDC discovery
// and JWKS objects whenever the fingerprint of the freshly rendered
// objects differs from the last recorded one. The fingerprint is
// content-based, so renderer-format changes (and any input that flows
// into the rendered output) trigger a re-publish even when the key ID is
// unchanged.
func (r *AWSWorkloadIdentityConfigReconciler) ensureOIDCObjectsPublished(ctx context.Context, config *identityv1.AWSWorkloadIdentityConfig, publicKeyPEM []byte, keyID string) error {
	currentHash, err := computePublishedIssuerHash(config.Status.IssuerURL(), publicKeyPEM, keyID)
	if err != nil {
	...
	if config.Status.PublishedIssuerHash == currentHash {
		return nil
	}

	log := logf.FromContext(ctx)
	log.V(1).Info("republishing self-hosted OIDC objects: stored fingerprint differs from current rendered output",
		"publishedIssuerHash.previous", config.Status.PublishedIssuerHash,
		"publishedIssuerHash.current", currentHash,
		"publishedKeyID.previous", config.Status.PublishedKeyID,
		"publishedKeyID.current", keyID,
	)
	...
	config.Status.PublishedIssuerHash = currentHash
	config.Status.PublishedKeyID = keyID

	return nil
}

// computePublishedIssuerHash returns a sha256 fingerprint of the OIDC
// objects that would be uploaded to S3 for the given inputs. The hash
// incorporates the rendered bytes (and key + content-type for each
// object), so any change to the renderer output or its inputs causes the
// hash to change, which the controller uses to decide whether a
// re-publish is required.
func computePublishedIssuerHash(issuerURL string, publicKeyPEM []byte, keyID string) (string, error) {
	objects, err := oidc.RenderIssuerObjects(issuerURL, publicKeyPEM, keyID)
	if err != nil {
		return "", fmt.Errorf("render OIDC issuer objects for hash: %w", err)
	}

	h := sha256.New()
	for _, obj := range objects {
		_, _ = fmt.Fprintf(h, "key:%s\nct:%s\nlen:%d\n", obj.Key, obj.ContentType, len(obj.Body))
		_, _ = h.Write(obj.Body)
		_, _ = h.Write([]byte{'\n'})
	}
