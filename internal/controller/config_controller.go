...
//
// Only the issuer path bubbles errors; runtime failures are surfaced via
// status conditions and a fixed retry from reconcileSelfHostedWebhookRuntime,
// not via a returned error.
func (r *AWSWorkloadIdentityConfigReconciler) reconcileSelfHostedComponents(
	...
	go func() {
		...
		runtimeResult = r.reconcileSelfHostedWebhookRuntime(ctx, log, runtimeConfig, operatorConfig, resolved)
	}()
...
// and reports the outcome via WebhookRuntimeReady. Failures are converted to
// False conditions and a fixed retry, never returned, so the caller can patch
// status without losing the issuer path's progress.
func (r *AWSWorkloadIdentityConfigReconciler) reconcileSelfHostedWebhookRuntime(ctx context.Context, log logr.Logger, config *identityv1.AWSWorkloadIdentityConfig, operatorConfig *identityv1.AWSWorkloadIdentityOperatorConfig, resolved *inventory.Resolution) ctrl.Result {
	if !resolved.Ready {
	...
	if err != nil {
		...
		log.Error(err, "self-hosted webhook runtime deferred", "awio.condition.reason", identityv1.ReasonRemoteClusterUnavailable)
	...
	if err != nil {
		...
		log.Error(err, "self-hosted webhook runtime deferred", "awio.condition.reason", identityv1.ReasonWebhookRuntimeApplyFailed)

		return ctrl.Result{RequeueAfter: webhookRuntimeRequeueAfter(outcome.Schedule, false)}
	}
	...
	return ctrl.Result{RequeueAfter: webhookRuntimeRequeueAfter(outcome.Schedule, true)}
}
