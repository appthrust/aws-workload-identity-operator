...
func TestApplyRemoteWebhookRuntimeCreatesMinimalResources(t *testing.T) {
	c := fakeClient(t, bootstrappedWebhookNamespace(), availableWebhookDeployment())
...
func TestApplyRemoteWebhookRuntimePreservesExistingResourceLabels(t *testing.T) {
	...
	c := fakeClient(t, bootstrappedWebhookNamespace(), deployment)
...
func TestApplyRemoteWebhookRuntimeWaitsForDeploymentBeforeCreatingWebhookConfiguration(t *testing.T) {
	c := fakeClient(t, bootstrappedWebhookNamespace())
...
func TestApplyRemoteWebhookRuntimeSurfacesUnmanagedMWHCBeforeDeploymentAvailable(t *testing.T) {
	...
	remoteClient := fakeClient(t, bootstrappedWebhookNamespace())
	...
	apiReader := fake.NewClientBuilder().
		...
		WithObjects(unlabeledMWHC, bootstrappedWebhookNamespace()).
		Build()
...
// TestApplyRemoteWebhookRuntimeSurfacesBootstrapPendingWhenNamespaceAbsent pins
// the contract that the chart-managed OCM ManifestWork (or equivalent external
// bootstrap) owns the workload-cluster Namespace. When the namespace is absent
// on the live API server, the controller must surface a level-triggered
// WebhookRuntimeReady=False condition with RemoteNamespaceBootstrapPending and
// must not issue a Namespace.Create — the delegated remote ServiceAccount has
// no cluster-wide namespaces:create verb under the chart's ClusterRole, so
// attempting Create would silently degrade to a Forbidden apply error rather
// than a clear precondition signal.
func TestApplyRemoteWebhookRuntimeSurfacesBootstrapPendingWhenNamespaceAbsent(t *testing.T) {
	c := fakeClient(t)

	outcome, err := applyRemoteWebhookRuntime(context.Background(), logr.Discard(), c, c, testWebhookNamespace, testPodIdentityWebhookImage)
	if err != nil {
		t.Fatalf("expected nil error for level-triggered bootstrap-pending branch, got %v", err)
	}

	if outcome.Condition.Ready {
		t.Fatalf("expected outcome.Condition.Ready=false when namespace is absent, got %#v", outcome.Condition)
	}

	if outcome.Condition.Reason != identityv1.ReasonRemoteNamespaceBootstrapPending {
		t.Fatalf("expected reason %q, got %q (message=%q)", identityv1.ReasonRemoteNamespaceBootstrapPending, outcome.Condition.Reason, outcome.Condition.Message)
	}

	ns := &corev1.Namespace{}
	if getErr := c.Get(context.Background(), client.ObjectKey{Name: testWebhookNamespace}, ns); !apierrors.IsNotFound(getErr) {
		t.Fatalf("expected namespace to remain absent (no Create attempt), got err=%v ns=%#v", getErr, ns)
	}
}

// TestEnsureRemoteNamespaceReturnsBootstrapPendingWhenAbsent pins the
// unit-level contract: the helper must return errRemoteNamespaceBootstrapPending
// without attempting a Create. Mirrors the rationale on
// errRemoteNamespaceBootstrapPending — the chart-managed OCM ManifestWork owns
// namespace bootstrap; cluster-wide namespaces:create would broaden the
// delegated SA beyond least privilege.
func TestEnsureRemoteNamespaceReturnsBootstrapPendingWhenAbsent(t *testing.T) {
	ctx := context.Background()
	c := fakeClient(t)

	op, err := ensureRemoteNamespace(ctx, c, c, testWebhookNamespace)
	if err == nil {
		t.Fatal("expected ensureRemoteNamespace to return errRemoteNamespaceBootstrapPending when namespace absent, got nil error")
	}

	if !isErrRemoteNamespaceBootstrapPending(err) {
		t.Fatalf("expected error to satisfy isErrRemoteNamespaceBootstrapPending, got %q", err.Error())
	}

	if op != controllerutil.OperationResultNone {
		t.Fatalf("expected OperationResultNone when refusing to create the namespace, got %s", op)
	}

	ns := &corev1.Namespace{}
	if getErr := c.Get(ctx, client.ObjectKey{Name: testWebhookNamespace}, ns); !apierrors.IsNotFound(getErr) {
		t.Fatalf("expected namespace to remain absent (no Create attempt), got err=%v ns=%#v", getErr, ns)
	}
}
...
// bootstrappedWebhookNamespace returns the workload-cluster Namespace as it
// would appear after the chart-managed OCM ManifestWork bootstrapped it: the
// managed-by/runtime labels are already stamped, so ensureRemoteNamespace
// passes the adoption probe without issuing a Create.
func bootstrappedWebhookNamespace() *corev1.Namespace {
