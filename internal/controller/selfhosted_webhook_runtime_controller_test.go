...
func TestSelfHostedWebhookRuntimeCacheSelectorMatchesRuntimeAndLegacyLabels(t *testing.T) {
	...
	if !selector.Matches(labels.Set(webhookRuntimeLabels(owner))) {
...
func TestScopedRuntimeObjectsUseLiveReadsForLegacyAdoption(t *testing.T) {
	...
	if _, _, _, _, err := ensureRemoteCASecret(ctx, hiddenClient, testWebhookNamespace, owner); !apierrors.IsAlreadyExists(err) {
	...
	op, caBundle, caCert, caKey, err := ensureRemoteCASecret(ctx, liveReadClient, testWebhookNamespace, owner)
	if err != nil {
		t.Fatal(err)
	}
...
func assertWebhookRuntimeLabels(t *testing.T, obj client.Object) {
	...
	if labels[identityv1.LabelManagedBy] != identityv1.ManagedByValue ||
		...
		labels[identityv1.LabelConfigUID] != testConfigUID ||
		labels[identityv1.LabelInventoryNS] != testInventoryNamespace ||
