...
func TestApplyRemoteWebhookRuntimeCreatesMinimalResources(t *testing.T) {
	...
	owner := webhookRuntimeOwner{ConfigUID: testConfigUID, InventoryNamespace: testInventoryNamespace}

	result, err := applyRemoteWebhookRuntime(context.Background(), logr.Discard(), c, owner, testWebhookNamespace, testPodIdentityWebhookImage)
	if err != nil {
		t.Fatal(err)
	}
...
func TestApplyRemoteWebhookRuntimeLabelsAreStableAcrossConfigOwners(t *testing.T) {
	ctx := context.Background()
	c := fakeClient(t, availableWebhookDeployment())
	ownerA := webhookRuntimeOwner{ConfigUID: testConfigUID, InventoryNamespace: testInventoryNamespace}
	ownerB := webhookRuntimeOwner{ConfigUID: testSiblingConfigUID, InventoryNamespace: testSiblingNamespace}

	if _, err := applyRemoteWebhookRuntime(ctx, logr.Discard(), c, ownerA, testWebhookNamespace, testPodIdentityWebhookImage); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := applyRemoteWebhookRuntime(ctx, logr.Discard(), c, ownerB, testWebhookNamespace, testPodIdentityWebhookImage); err != nil {
		t.Fatal(err)
	}
	...
	if !reflect.DeepEqual(firstLabels, second.Labels) {
		t.Fatalf("expected Deployment labels to be stable across Config owners: first=%#v second=%#v", firstLabels, second.Labels)
	}

	if !reflect.DeepEqual(firstPodLabels, second.Spec.Template.Labels) {
		t.Fatalf("expected PodTemplate labels to be stable across Config owners: first=%#v second=%#v", firstPodLabels, second.Spec.Template.Labels)
	}

	if !reflect.DeepEqual(firstPodAnnotations, second.Spec.Template.Annotations) {
		t.Fatalf("expected PodTemplate annotations to be stable across Config owners: first=%#v second=%#v", firstPodAnnotations, second.Spec.Template.Annotations)
	}
}
...
func TestApplyRemoteWebhookRuntimeRemovesLegacyConfigSpecificLabels(t *testing.T) {
	...
	if _, err := applyRemoteWebhookRuntime(ctx, logr.Discard(), c, webhookRuntimeOwner{ConfigUID: testConfigUID, InventoryNamespace: testInventoryNamespace}, testWebhookNamespace, testPodIdentityWebhookImage); err != nil {
		t.Fatal(err)
	}
...
func TestEnsureRemoteDeploymentPreservesUnmanagedTemplateFields(t *testing.T) {
	...
	_, op, err := ensureRemoteDeployment(ctx, c, testWebhookNamespace, testWebhookImageNew, owner, "fingerprint-a")
	if err != nil {
		t.Fatal(err)
	}
...
func TestEnsureRemoteDeploymentNoopsAfterManagedFieldsConverge(t *testing.T) {
	...
	if _, _, err := ensureRemoteDeployment(ctx, c, testWebhookNamespace, testWebhookImageNew, owner, "fingerprint-a"); err != nil {
		t.Fatal(err)
	}
