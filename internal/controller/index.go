...
// annotation. Cross-namespace OwnerReferences are not valid in Kubernetes
// (parent lives in a user namespace, children live in per-cluster namespaces),
// so label/annotation-based ownership is the only viable signal for this fleet
// binding. The annotation provides a cheap namespaced-name index lookup;
// callers MUST then apply the in-memory ownership predicate
// (replicaSetOwnsChild) which additionally requires the managed-by label and
// the replicaset-uid label to match the live parent's UID, so a previous RS
// incarnation's surviving children are filtered out of the owned list.
...
// newReplicaSetChildTemplate. The annotation alone is not sufficient to claim
// ownership — see replicaSetOwnsChild for the additional UID match that
// distinguishes the live parent incarnation from a stale one with the same
// namespace/name.
