...
func TestRequestsForListLogsListFailureAndReturnsNil(t *testing.T) {
	...
	// NS-WATCH-MAP-OBS-03: a LIST failure inside requestsForList MUST
	// increment awio_watch_map_list_errors_total exactly once with stable
	// labels so on-call can alert before the dropped event is silently
	// logged-and-discarded. The metric is sampled with a before/after delta
	// because the shared ctrlmetrics.Registry is process-global and other
	// tests can mutate it in parallel.
	before := watchMapListErrorCountFor(t,
		metrics.ControllerRole,
		"rolesForOperatorConfig",
		"AWSWorkloadIdentityOperatorConfig",
	)

	requests := requestsForList(context.Background(), log, failingListReader{err: listErr}, metrics.ControllerRole, "rolesForOperatorConfig", "AWSWorkloadIdentityOperatorConfig", &identityv1.AWSServiceAccountRoleList{})
	if requests != nil {
		t.Fatalf("expected nil requests on list error, got %#v", requests)
	}

	after := watchMapListErrorCountFor(t,
		metrics.ControllerRole,
		"rolesForOperatorConfig",
		"AWSWorkloadIdentityOperatorConfig",
	)
	if delta := after - before; delta != 1 {
		t.Fatalf("expected watchMapListErrorsTotal delta 1 for LIST failure, got %v (before=%v after=%v)", delta, before, after)
	}
...
func TestRequestsForListLogsExtractFailureAndReturnsNil(t *testing.T) {
	...
	// NS-WATCH-MAP-OBS-03 regression assert: extract failures are a distinct
	// failure mode from LIST failures and MUST NOT increment
	// awio_watch_map_list_errors_total. If a future change promotes extract
	// failures into the same counter without a separate observability path,
	// this delta==0 check fires so the decision is explicit rather than
	// silently merging unrelated failure rates onto the same on-call signal.
	before := watchMapListErrorCountFor(t,
		metrics.ControllerConfig,
		"configsForOperatorConfig",
		"AWSWorkloadIdentityOperatorConfig",
	)

	requests := requestsForList(context.Background(), log, failingListReader{}, metrics.ControllerConfig, "configsForOperatorConfig", "AWSWorkloadIdentityOperatorConfig", &malformedObjectList{})
	if requests != nil {
	...
	if delta := after - before; delta != 0 {
		t.Fatalf("expected watchMapListErrorsTotal delta 0 for extract failure (LIST succeeded), got %v (before=%v after=%v)", delta, before, after)
	}
...
// watchMapListErrorCountFor reads the current value of
// awio_watch_map_list_errors_total{controller,map_func,kind} from the shared
// controller-runtime registry. Unlike the role_replicaset_controller_test.go
// helper (whose controller label is hard-pinned to ControllerRoleReplicaSet),
// this variant takes controller as an argument so common_test.go can sample
// the counter for any caller of requestsForList — the four production sites
// span ControllerRole and ControllerConfig. We deliberately introduce a
// local helper rather than generalize the existing one to keep the 1-violation
// 1-commit blast radius limited to common_test.go. Returns 0 when the series
// has not been recorded yet — counters with zero observations are absent
// from Gather(). The kind argument intentionally stays parameterised even
// though both current call sites pass the same watchKind: it preserves label
// symmetry with controller/mapFunc and lets future tests assert on the
// other three production callers without further helper churn.
//
//nolint:unparam // kind argument is preserved for the label-symmetry / future-test reason documented above.
func watchMapListErrorCountFor(t *testing.T, controller, mapFunc, kind string) float64 {
	t.Helper()
	...
	for _, family := range families {
		...
		for _, m := range family.GetMetric() {
			if metricLabelValue(m.GetLabel(), "controller") != controller {
				continue
			}

			if metricLabelValue(m.GetLabel(), "map_func") != mapFunc {
				continue
			}

			if metricLabelValue(m.GetLabel(), "kind") != kind {
				continue
			}
...
// metricLabelValue is the local counterpart to predicates_test.go's labelValue.
// Duplicated (not re-exported) so common_test.go does not develop a cross-file
// dependency that another agent could trip over while editing predicates_test.go
// in parallel on the shared working tree.
func metricLabelValue(pairs []*dto.LabelPair, name string) string {
