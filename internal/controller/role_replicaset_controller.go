...
//
// +kubebuilder:rbac:groups=aws.identity.appthrust.io,resources=awsserviceaccountrolereplicasets,verbs=get;list;watch;patch
...
// +kubebuilder:rbac:groups=aws.identity.appthrust.io,resources=awsserviceaccountroles,verbs=get;list;watch;create;patch;delete
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) reconcileReplicaSetNormal(ctx context.Context, log logr.Logger, rs *identityv1.AWSServiceAccountRoleReplicaSet) (ctrl.Result, error) {
	...
	applyErr := r.applyReplicaSetRolloutClusters(ctx, log, rs, resolution, rollout, childrenByNS, template)
	pruned, pruneErr := r.pruneReplicaSetChildren(ctx, children, resolution.clusters)
	rs.Status.StaleClusterCount += countInt32(pruned)

	setReplicaSetAggregateConditions(rs)

	if applyErr != nil || pruneErr != nil {
		// Surface transient child-apply errors so controller-runtime applies
		// its rate-limited backoff. Returning a non-zero RequeueAfter
		// alongside an error would short-circuit the rate limiter and
		// busy-loop on a misbehaving apiserver.
		patchErr := r.patchReplicaSetStatus(ctx, log, rs, beforeStatus)
		return ctrl.Result{}, errors.Join(applyErr, pruneErr, patchErr)
	}
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) applyReplicaSetRolloutClusters(ctx context.Context, log logr.Logger, rs *identityv1.AWSServiceAccountRoleReplicaSet, resolution replicaSetPlacementResolution, rollout replicaSetRolloutPlan, childrenByNS map[string]*identityv1.AWSServiceAccountRole, template *replicaSetChildTemplate) error {
	...
	for _, clusterName := range sets.List(resolution.clusters) {
		...
		if rollout.rolloutClusters.Has(clusterName) {
			applied, err := r.applyReplicaSetCluster(ctx, log, rs, clusterName, summary, childrenByNS[clusterName], template)
			summary = applied
			if err != nil {
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) reconcileReplicaSetDelete(ctx context.Context, log logr.Logger, rs *identityv1.AWSServiceAccountRoleReplicaSet) (ctrl.Result, error) {
	...
	for _, child := range children {
		...
		if err := client.IgnoreNotFound(r.Delete(ctx, child)); err != nil {
			wrapped := fmt.Errorf("delete AWSServiceAccountRole %s/%s: %w", child.Namespace, child.Name, err)
			// Surface CleanupBlocked before returning so observers see the stuck state during the rate-limited retry window.
			rs.Status.ObservedGeneration = rs.Generation
			setCondition(&rs.Status.Conditions, rs.Generation, identityv1.ConditionCleanupBlocked, metav1.ConditionTrue, identityv1.ReasonChildApplyFailed, wrapped.Error())
			setCondition(&rs.Status.Conditions, rs.Generation, identityv1.ConditionReady, metav1.ConditionFalse, identityv1.ReasonChildApplyFailed, wrapped.Error())
			if patchErr := r.patchReplicaSetStatus(ctx, log, rs, beforeStatus); patchErr != nil {
				log.Error(patchErr, "failed to patch CleanupBlocked status during child delete")
			}
	...
	// remaining == 0: our cleanup is unblocked. Emit CleanupBlocked=False once
	// (mirroring clearRoleDeletionBlocked) so observers see the transition even
	// when an unrelated finalizer keeps the ReplicaSet visible after this Patch.
	if meta.IsStatusConditionTrue(rs.Status.Conditions, identityv1.ConditionCleanupBlocked) {
		rs.Status.ObservedGeneration = rs.Generation
		setCondition(&rs.Status.Conditions, rs.Generation, identityv1.ConditionCleanupBlocked, metav1.ConditionFalse, identityv1.ReasonDeletionUnblocked, "all generated AWSServiceAccountRole children have been deleted")
		if err := r.patchReplicaSetStatus(ctx, log, rs, beforeStatus); err != nil {
			return ctrl.Result{}, err
		}
	}
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) applyReplicaSetCluster(ctx context.Context, log logr.Logger, rs *identityv1.AWSServiceAccountRoleReplicaSet, clusterName string, planSummary identityv1.AWSServiceAccountRoleClusterSummary, child *identityv1.AWSServiceAccountRole, template *replicaSetChildTemplate) (identityv1.AWSServiceAccountRoleClusterSummary, error) {
	...
	if child == nil {
		...
		if err := r.Create(ctx, created); err != nil {
			...
			return summary, fmt.Errorf("create AWSServiceAccountRole %s: %w", key.String(), err)
		}
	...
	if !sameServiceAccountSubject(child.Spec.ServiceAccount, rs.Spec.Template.Spec.ServiceAccount) {
		if child.DeletionTimestamp.IsZero() {
			if err := client.IgnoreNotFound(r.Delete(ctx, child)); err != nil {
				...
				return summary, fmt.Errorf("delete stale AWSServiceAccountRole %s: %w", key.String(), err)
			}
		}
	...
	if template.drifts(child) {
		...
		if err := r.Patch(ctx, updated, client.MergeFromWithOptions(child, client.MergeFromWithOptimisticLock{})); err != nil {
			...
			return summary, fmt.Errorf("patch AWSServiceAccountRole %s: %w", key.String(), err)
		}
...
// Returns the number of pruned children plus any aggregated delete errors so
// that controller-runtime applies its rate-limited backoff on transient
// apiserver failures.
func (r *AWSServiceAccountRoleReplicaSetReconciler) pruneReplicaSetChildren(ctx context.Context, children []*identityv1.AWSServiceAccountRole, selected sets.Set[string]) (int, error) {
	pruned := 0
	var errs error
	for _, child := range children {
		...
		if err := client.IgnoreNotFound(r.Delete(ctx, child)); err != nil {
			errs = errors.Join(errs, fmt.Errorf("delete stale AWSServiceAccountRole %s/%s: %w", child.Namespace, child.Name, err))
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) listOwnedReplicaSetChildren(ctx context.Context, rs *identityv1.AWSServiceAccountRoleReplicaSet) ([]*identityv1.AWSServiceAccountRole, error) {
	ownerKey := client.ObjectKeyFromObject(rs)
	list := &identityv1.AWSServiceAccountRoleList{}
	if err := r.List(ctx, list, roleByReplicaSetOwnerRefKey(ownerKey)); err != nil {
		return nil, fmt.Errorf("list AWSServiceAccountRoles by ReplicaSet owner-ref %q: %w", ownerKey.String(), err)
	}
...
func applyReplicaSetRolloutStatus(rs *identityv1.AWSServiceAccountRoleReplicaSet, rollout replicaSetRolloutPlan) {
	...
	rs.Status.DesiredClusterCount = countInt32(int(rollout.summary.Updating) + int(rollout.summary.Succeeded))
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	builder := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&corev1.Namespace{},
			handler.EnqueueRequestsFromMapFunc(r.replicaSetsForNamespace),
			crbuilder.WithPredicates(namespaceAppearedPredicate())).
...
// namespaceAppearedPredicate keeps only Namespace CreateEvents. ReplicaSet
// reconciliation only cares about a namespace's appearance — once present,
// per-cluster status changes are driven by AWSServiceAccountRole child events
// (already watched). Dropping Update/Delete/Generic avoids hot reconciliation
// on unrelated namespace status/label churn fleet-wide.
func namespaceAppearedPredicate() predicate.Predicate {
	return metricsRecordingPredicate(metrics.ControllerRoleReplicaSet, predicateKeepFns{
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) replicaSetsForChildRole(ctx context.Context, obj client.Object) []reconcile.Request {
	ownerRef := obj.GetAnnotations()[identityv1.AnnotationReplicaSetOwnerRef]
	if ownerRef == "" {
		// Child carries no parent stamp: it is not owned by any
		// AWSServiceAccountRoleReplicaSet. newReplicaSetChildTemplate always
		// stamps AnnotationReplicaSetOwnerRef on children we manage, so the
		// absence of the annotation means the event is for a foreign
		// AWSServiceAccountRole. Nothing to enqueue; avoid an unbounded LIST.
		return nil
	}

	nn, err := namespacedNameFromString(ownerRef)
	if err != nil {
		// Malformed stamp: log at V(1) and drop. We deliberately do NOT fall
		// back to a cluster-wide LIST — that would re-introduce the cache
		// anti-pattern documented at
		// sigs.k8s.io/controller-runtime/pkg/handler/enqueue_mapped.go.
		log := watchMapLogger(ctx, metrics.ControllerRoleReplicaSet, "replicaSetsForChildRole", "AWSServiceAccountRole", obj)
		log.V(1).Info("ignoring child with malformed replicaset owner-ref annotation", "error", err.Error())
		return nil
	}
...
// replicaSetsForNamespace fans out a Namespace CreateEvent to every
// AWSServiceAccountRoleReplicaSet that is currently parked on
// ReasonClusterNamespaceMissing for that exact namespace. This is the retry
// path for the existingReplicaSetClusterSummary code that sets per-cluster
// Failed when the target namespace does not yet exist — without this watch,
// the ReplicaSet would never be re-enqueued when the namespace later appears.
//
// The map is narrow on purpose: we LIST cluster-scoped because
// AWSServiceAccountRoleReplicaSet may live in any namespace, but we only emit
// a Request when the ReplicaSet's status records the exact namespace name as
// missing. ReplicaSets that already see the namespace contribute no Request,
// so unrelated Namespace events do not cause reconciliation churn.
func (r *AWSServiceAccountRoleReplicaSetReconciler) replicaSetsForNamespace(ctx context.Context, obj client.Object) []reconcile.Request {
	log := watchMapLogger(ctx, metrics.ControllerRoleReplicaSet, "replicaSetsForNamespace", "Namespace", obj)
	...
	if err := r.List(ctx, replicaSets); err != nil {
		// EnqueueRequestsFromMapFunc cannot return errors; a LIST failure here
		// would silently drop the namespace-appeared event and leave parked
		// ReplicaSets Failed forever. Log loudly so the failure is observable.
		metrics.RecordWatchMapListError(metrics.ControllerRoleReplicaSet, "replicaSetsForNamespace", "Namespace")
		log.Error(err, "failed to list ReplicaSets for namespace watch map; dropping event")
		return nil
	}
	...
	for i := range replicaSets.Items {
		...
		if replicaSetWaitingOnNamespace(rs, namespaceName) {
...
// replicaSetWaitingOnNamespace reports whether the ReplicaSet currently has a
// per-cluster summary parked on ReasonClusterNamespaceMissing for the given
// namespace name. ClusterName mirrors the workload cluster namespace in this
// controller's data model (see existingReplicaSetClusterSummary).
func replicaSetWaitingOnNamespace(rs *identityv1.AWSServiceAccountRoleReplicaSet, namespaceName string) bool {
	return slices.ContainsFunc(rs.Status.Clusters, func(summary identityv1.AWSServiceAccountRoleClusterSummary) bool {
		return summary.ClusterName == namespaceName && summary.Reason == identityv1.ReasonClusterNamespaceMissing
	})
}
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) replicaSetsForOCMPlacement(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	return requestsForReplicaSetsReferencingPlacement(ctx, log, r.Client, obj.GetNamespace(), obj.GetName(), "replicaSetsForOCMPlacement", "Placement")
}
...
func (r *AWSServiceAccountRoleReplicaSetReconciler) replicaSetsForOCMPlacementDecision(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	return requestsForReplicaSetsReferencingPlacement(ctx, log, r.Client, obj.GetNamespace(), placementName, "replicaSetsForOCMPlacementDecision", "PlacementDecision")
}

func requestsForReplicaSetsReferencingPlacement(ctx context.Context, log logr.Logger, c client.Reader, namespace, name, mapFunc, kind string) []reconcile.Request {
	if name == "" {
		// Empty placement name cannot match any ReplicaSet; skip the indexed
		// lookup. PlacementDecision events without the placement label hit
		// this path via replicaSetsForOCMPlacementDecision.
		return nil
	}

	replicaSets := &identityv1.AWSServiceAccountRoleReplicaSetList{}
	if err := c.List(ctx, replicaSets, client.InNamespace(namespace), replicaSetByPlacementRefKey(name)); err != nil {
		// Indexed LIST failure (e.g., indexer registration drift) would
		// otherwise silently drop Placement/PlacementDecision watch events
		// because EnqueueRequestsFromMapFunc map functions cannot return
		// errors. Fall back to a namespace-scoped non-indexed LIST and
		// filter in-memory. Scope stays namespace-bound — we deliberately
		// do NOT widen to a cluster-wide LIST.
		metrics.RecordWatchMapListError(metrics.ControllerRoleReplicaSet, mapFunc, kind)
		log.Error(err, "failed to list ReplicaSets for placement watch map; attempting namespace-scoped fallback")

		fallback := &identityv1.AWSServiceAccountRoleReplicaSetList{}
		if fbErr := c.List(ctx, fallback, client.InNamespace(namespace)); fbErr != nil {
			metrics.RecordWatchMapListError(metrics.ControllerRoleReplicaSet, mapFunc, kind)
			log.Error(fbErr, "fallback namespace-scoped list also failed; dropping placement watch event")
			return nil
		}

		requests := make([]reconcile.Request, 0, len(fallback.Items))
		...
		log.V(1).Info("used non-indexed fallback list for placement watch map", "placement", name, "namespace", namespace, "fallbackMatches", len(requests))
	...
	for i := range replicaSets.Items {
		rs := &replicaSets.Items[i]
		// The index already filters by placementRefs[*].name; this defensive
		// check protects against future index/IndexerFunc drift.
		if replicaSetReferencesPlacement(rs, name) {
			requests = append(requests, reconcile.Request{NamespacedName: client.ObjectKeyFromObject(rs)})
		}
	}
...
func replicaSetOwnsChild(rs *identityv1.AWSServiceAccountRoleReplicaSet, child *identityv1.AWSServiceAccountRole) bool {
	// Ownership requires all three signals stamped by newReplicaSetChildTemplate:
	//   * managed-by label    — claim by this operator
	//   * owner-ref annotation — namespaced-name of the parent ReplicaSet
	//   * replicaset-uid label — UID of *this* parent incarnation
	//
	// metadata.uid is the canonical instance identifier in Kubernetes
	// (api-conventions.md "Names": uid distinguishes objects with the same
	// name that have been deleted and recreated). Cross-namespace
	// metav1.OwnerReference is not legal — the parent lives in the operator
	// namespace and children live in per-cluster namespaces — so the UID is
	// carried as a label on the child instead of in an OwnerReference. A
	// previous RS incarnation deleted and recreated under the same
	// namespace/name yields a fresh UID at the apiserver, so its surviving
	// children (which still carry the old UID label) must NOT be re-adopted
	// by the new parent. The probe path in existingReplicaSetClusterSummary
	// surfaces such surviving children as Conflict instead of silently
	// taking them over.
	...
	if labels[identityv1.LabelReplicaSetUID] != string(rs.UID) {
		return false
	}

	return child.GetAnnotations()[identityv1.AnnotationReplicaSetOwnerRef] == client.ObjectKeyFromObject(rs).String()
}
...
func setReplicaSetAggregateConditions(rs *identityv1.AWSServiceAccountRoleReplicaSet) {
	...
	default:
		// Applied=True must hold only when every desired child has been
		// issued to the apiserver. Clusters parked on RolloutPending and
		// clusters skipped on cache-lag AlreadyExists do not increment
		// AppliedClusterCount, so a strict equality check against
		// DesiredClusterCount is the honest aggregate signal.
		if rs.Status.AppliedClusterCount == rs.Status.DesiredClusterCount {
			setCondition(&rs.Status.Conditions, rs.Generation, identityv1.ConditionAWSServiceAccountRolesApplied, metav1.ConditionTrue, identityv1.ReasonChildrenApplied, "generated AWSServiceAccountRole children are applied")
		} else {
			setCondition(&rs.Status.Conditions, rs.Generation, identityv1.ConditionAWSServiceAccountRolesApplied, metav1.ConditionFalse, identityv1.ReasonChildrenPending, childrenPendingMessage)
		}
		setCondition(&rs.Status.Conditions, rs.Generation, identityv1.ConditionAWSServiceAccountRolesReady, metav1.ConditionFalse, identityv1.ReasonChildrenPending, childrenPendingMessage)
...
func firstReservedKey(values map[string]string, reserved sets.Set[string]) string {
	for _, key := range sets.List(reserved) {
...
var (
	reservedReplicaSetTemplateLabels = sets.New[string](
	...
	reservedReplicaSetTemplateAnnotations = sets.New[string](
		identityv1.AnnotationReplicaSetOwnerRef,
	)
)

// hasMapping reports whether gvk is registered. Only meta.IsNoMatchError is
// treated as "CRD genuinely absent" — every other RESTMapper failure
// (discovery transport, RBAC, ambiguous kind) is surfaced so the optional
// wire-up does not silently disappear when the CRD actually exists.
func hasMapping(rm meta.RESTMapper, gvk schema.GroupVersionKind) (bool, error) {
	...
	case meta.IsNoMatchError(err):
		return false, nil
	default:
		return false, fmt.Errorf("probe REST mapping for %s: %w", gvk, err)
	}
}
