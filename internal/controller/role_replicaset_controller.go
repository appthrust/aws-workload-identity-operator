...
func replicaSetOwnsChild(rs *identityv1.AWSServiceAccountRoleReplicaSet, child *identityv1.AWSServiceAccountRole) bool {
	// Ownership is established by the (managed-by, owner-ref annotation) pair.
	// The owner-ref annotation is stamped once at creation by
	// newReplicaSetChildTemplate and is stable across parent UID changes,
	// unlike LabelReplicaSetUID which can drift (external mutation, or parent
	// RS recreated with same name but new UID). Cross-namespace OwnerReferences
	// are not valid in Kubernetes, so this annotation is the primary ownership
	// signal. LabelReplicaSetUID is still stamped on children for forensics and
	// remains a reserved template key (defense in depth).
