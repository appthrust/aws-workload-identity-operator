...
func TestRoleReplicaSetPlacementResolveErrorPreservesAccumulatedStatus(t *testing.T) {
	...
	rs := testRoleReplicaSet()
	rs.Generation = 5
	rs.Spec.PlacementRefs = []identityv1.PlacementReference{{
		APIGroup: placementAPIGroupOCM,
		Kind:     placementKindOCM,
		Name:     "missing",
	}}
...
func testRoleReplicaSet() *identityv1.AWSServiceAccountRoleReplicaSet {
	return &identityv1.AWSServiceAccountRoleReplicaSet{
		...
		Spec: identityv1.AWSServiceAccountRoleReplicaSetSpec{
			PlacementRefs: []identityv1.PlacementReference{{
				APIGroup: placementAPIGroupOCM,
				Kind:     placementKindOCM,
				Name:     "prod",
			}},
...
func testOCMPlacement() *unstructured.Unstructured {
	placement := newUnstructured(placementGVKOCM)
	placement.SetNamespace(testReplicaSetNamespace)
	placement.SetName("prod")
	placement.SetUID("22222222-2222-2222-2222-222222222222")

	return placement
}

func testOCMPlacementDecision(placementUID types.UID, clusterNames ...string) *unstructured.Unstructured {
	decision := newUnstructured(placementDecisionGVKOCM)
	decision.SetNamespace(testReplicaSetNamespace)
	decision.SetName("prod-decision-1")
	decision.SetLabels(map[string]string{placementLabelOCM: "prod"})
	decision.SetOwnerReferences([]metav1.OwnerReference{{
		APIVersion: placementAPIVersionOCM,
		Kind:       placementKindOCM,
		Name:       "prod",
		UID:        placementUID,
	}})

	decisions := make([]any, 0, len(clusterNames))
	for _, clusterName := range clusterNames {
		decisions = append(decisions, map[string]any{"clusterName": clusterName})
	}
	_ = unstructured.SetNestedSlice(decision.Object, decisions, "status", "decisions")
